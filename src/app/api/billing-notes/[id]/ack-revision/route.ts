 ...
 *   "รับทราบว่าใบเสนอ Rev ใหม่แล้ว — ยอดเดิมถูกแล้ว" → ป้ายเตือนหายจนกว่าจะ Rev อีกรอบ (0133)
 *
...
export const POST = withRoute(async (_req: Request, { params }: { params: { id: string } }) => {
  ...
  if (error && /ack_revision_no/i.test(error.message ?? ""))
    return err("ยังไม่ได้รัน migration 0133 (แก้เอกสารได้ตลอด) — รันก่อนใช้งาน", 400);
