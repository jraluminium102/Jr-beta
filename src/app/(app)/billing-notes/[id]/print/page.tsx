          ...
          {installments.map((it) => {
            ...
            return (
              <tr key={it.id}>
                ...
                <td className="p-2 border border-gray-200 text-right align-top tabular-nums">{baht(base)}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div className="flex justify-end mt-4">
        <table className="text-sm">
          <tbody>
            ...
            <tr className="font-bold text-lg" style={{ color: "#7d0f15" }}><td className="pr-10 py-1 border-t text-left">{isSingle ? "ยอดชำระ" : (whtAmt > 0 ? "ยอดชำระสุทธิ" : "ยอดรวมทั้งสิ้น")}</td><td className="text-right border-t tabular-nums">฿{baht(effTotal)}</td></tr>
          </tbody>
        </table>
      </div>
