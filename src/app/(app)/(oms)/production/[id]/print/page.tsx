...
// Production status display mapping
...
export default async function ProductionPrintPage({
  ...
  // 2) Find latest BOQ for this job
  let boqItems: {
    id: number;
    category: string;
    name: string;
  ...
  if (job?.id) {
    ...
    if (latestBoq) {
      ...
      const { data: items } = await supabase
        .from("boq_items")
        .select("id, category, name, spec, qty, unit, sort_order")
        .eq("boq_id", latestBoq.id)
        .order("sort_order", { ascending: true });

      boqItems = (items ?? []) as typeof boqItems;
    }
  }
    ...
    <div className="min-h-dvh bg-gray-100 print:bg-white">
      ...
      >
            ...
            <table className="mt-2 ml-auto" style={{ fontSize: 12 }}>
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {STATUS_LABEL[prodRow.status as string] ??
        ...
        <div
          ...
          style={{ color: "#b3151d", fontSize: 14 }}
        >
          รายการวัสดุ / ชิ้นงาน
        </div>
            ...
            <tbody>
              {boqItems.map((item, i) => (
                ...
                >
                  ...
                  <td className="p-2 border border-gray-200 align-top font-medium">
                    {item.name || "—"}
                  </td>
                  <td
                    className="p-2 border border-gray-200 align-top"
                  ...
                  {/* checkbox column for technician */}
                  <td className="p-2 border border-gray-200 text-center align-top">
                    <div
                      style={{
                        width: 18,
