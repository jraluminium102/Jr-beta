...
// isSales: true = SALES role → force ?my=1, ซ่อน filter ที่ไม่เกี่ยว
export default function FollowUpClient({ isSales }: { isSales: boolean }) {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["followup", q, phase, onlyIssue, onlyOverdue, isSales],
    queryFn: () => {
      ...
      // SALES: ส่ง my=1 อัตโนมัติ — เห็นเฉพาะงานตัวเองเท่านั้น (privacy ข้อมูลลูกค้า)
      if (isSales) p.set("my", "1");
    ...
    <div className="p-4 sm:p-6 fade-in">
      <div className="mb-4 sm:mb-5">
        ...
        <p className="text-sm" style={{ color: "var(--t-low)" }}>
          {isSales ? "งานของคุณ — เฟสปัจจุบัน ค้างนานไหม ติดปัญหาอะไร" : "ลูกค้าแต่ละรายอยู่เฟสไหน ค้างนานไหม ติดปัญหาอะไร"}
        </p>
      </div>
            ...
            {rows.map((r) => (
              <button key={r.id} onClick={() => setOpenId(r.id)} className="focusable pressable w-full text-left glass-card rounded-2xl p-4">
                ...
                <div className="flex items-center gap-3 mt-2 text-[12px]">
                  <span className={cn("tnum", r.overdue ? "text-amber-300 font-semibold" : "text-white/70")}>ค้าง {r.days_in_phase} วัน{r.overdue ? " เกิน" : ""}</span>
