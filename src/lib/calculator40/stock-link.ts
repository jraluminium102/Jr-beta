...
// ─────────────────────────────────────────────────────────────
// ⚠ ไฟล์นี้ทับเฉพาะ "ราคา" (pricebook) เท่านั้น — ทับ "โครงสูตร" (รหัส/จำนวน/ความยาว/เพิ่ม-ลดบรรทัด)
//   อยู่คนละชั้น: ดู applyLineOverrides() ใน ./line-overrides.ts (0134) แล้วประกบเข้ากับ PRODUCTS/CUT_SPEC_BY_ID
//   ตรงจุดที่โค้ดฝั่งเรียกใช้อยู่แล้ว (ไม่รวมเข้าที่นี่ เพราะฝั่งใบตัดมีฟังก์ชันจริงในบรรทัด
//   ส่งข้าม server→client component ไม่ได้ — ดูคอมเมนต์หัวไฟล์ line-overrides.ts)
...
export function buildPriceOverride(rows: StockRow[], pb: any = PB): PriceOverride {
  ...
  for (const r of rows || []) {
    ...
    if (cost > 0) {
      ...
      //   ⚠ อ่านรหัสจากชื่อได้ "เฉพาะชื่อที่ขึ้นต้นด้วยรหัส" เท่านั้น (สโตร์จริงตั้งชื่อแบบนี้ทั้งหมด
      //     — ตรวจแล้ว 730/730 แถวที่มีรหัส ขึ้นต้นด้วยรหัสหมด เช่น "F7855-ชนกลางบานเลื่อนรับแรง-อบขาว")
      //   แถวที่รหัสอยู่กลางชื่อ/ในวงเล็บ เช่น "ชนกลาง (F7855)" หรือ "วงกบ 3 ด้าน F7859"
      //     = แถวที่คนสร้างเองทีหลัง ไม่ใช่เส้นอลูรายสี → ห้ามเอามาตั้งราคาให้รหัสนั้น
      //     ไม่งั้นราคาสีของจริง (7 สี) ถูกแถวเดียวที่ไม่มีสีทับ (เจ้าของจับได้ 3 ก.ย.69 "ทำไมดึงตัวนี้ไปคิดราคา")
      //   หมายเหตุ: แถวพวกนั้นยังผูกราคาผ่าน PARTS(ชื่อ) ได้ตามเดิม — ไม่ได้ตัดทิ้ง
      const nameHead = /^([FBE]\d{4,5}[A-Za-z]?)\b/.exec(name);
      const nameCodes = nameHead ? [normCode(nameHead[1])] : [];
