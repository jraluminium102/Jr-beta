...
export const ALU_FROM_CUTLIST: Record<string, string> = {
  // ⛔ ห้ามใส่ roof: "awning" — เคยใส่แล้วทุนหายมหาศาล (2 ก.ย.69 ใส่ตอนเช้า ถอนตอนบ่าย)
  //   เจ้าของขอ "จำนวนแผ่นอิงไฟล์ตัดประกอบ" → ผมสลับ roof มาดึงจากใบตัด anchor 407 ผ่านหมด เลยคิดว่าปลอดภัย
  //   แต่ anchor ทดสอบแค่ "ไวนิล" (ค่าตั้งต้น) — พอไล่ครบทั้ง 19 ชนิดแผ่นถึงเจอว่าพังยับ:
  //     ใบตัดรู้จักแค่ 6 ชนิด (ไวนิล/ดีไลท์/เมทัลชีท/โพลีตัน/ชินโคร์ HC/Sup)
  //     อีก 13 ชนิดที่คิดราคาแตกละเอียดกว่า (เมทัล 8 แบบ · ชินโคร์ 4 เกรด · กระจก 2) หาราคาไม่เจอ
  //     → ไม่ออกบรรทัดแผ่นเลย = ค่าแผ่นหลังคาหายทั้งก้อน
  //   ทุนที่หายจริง: ชินโคร์ Grand 10มม −54,442 (−83%) · เมทัล 2" เหล็ก-PU −17,486 · กระจก 5+5 −10,930
  //     แม้แต่ไวนิลที่ anchor ทดสอบก็ยังหาย 770 (ฝาครอบ)
  //   ถ้าจะทำใหม่ ต้องแมป 19 ชนิดของคิดราคา → 6 ชนิดของใบตัด ให้ครบก่อน แล้วเทสทีละชนิด
...
/**
 * ชื่อแถวใบตัดที่ "ไม่มีรหัสสโตร์" → ชื่อที่ฝั่งคิดราคาใช้เรียกของชิ้นเดียวกัน
 * ใช้ให้หน้าเทียบจับคู่สองฝั่งได้ (ไฟล์เขียน "ราง (เท่ากว้าง)" · คิดราคาเรียก "รางน้ำอลู ขอบต่ำ …")
 * ไม่รู้จัก = คืนชื่อเดิม (ตัดเลขด้าน/เลขตำแหน่งออก) — เจ้าของจะได้เห็นคำที่ไฟล์เขียนจริง ๆ
 */
/**
 * ตารางเทียบชื่อ "ของชิ้นเดียวกัน" ที่ใบตัดกับคิดราคาเรียกคนละคำ (เจ้าของสั่ง 2 ก.ย.69 "ผูกวัสดุให้ถูก")
 * ⚠ ใส่ได้เฉพาะคู่ที่ยืนยันแล้วว่าเป็นของชิ้นเดียวกันจริง — ห้ามจับคู่เพื่อไล่ให้เขียว
 *   (เคยพลาดมาแล้วแบบตรงข้าม: ไปเติมของเข้าใบตัดเพื่อให้ตรงกับคิดราคา — บานติดตาย 21 ส.ค.69)
 */
const CUT_NAME_ALIAS: Record<string, { match: RegExp; as: string }[]> = {
  // ใบตัดเรียกตามตำแหน่ง (เสารับบาน) · คิดราคาเรียกตามขนาดกล่อง (เสารับ 1"x4") — ของชิ้นเดียวกัน
  topslide: [{ match: /^เสารับบาน/, as: 'เสารับ 1"x4"' }],
  // ใบตัดเขียนเต็ม "ฝาครอบรางบน" · คิดราคาเขียนย่อ "ฝาครอบราง"
  pcdoor: [{ match: /^ฝาครอบรางบน/, as: "ฝาครอบราง" }],
};

export function uncodedDisplayName(rawName: string, opt?: { prodId?: string; material?: string }): string {
  const nm = String(rawName ?? "");
  const alias = opt?.prodId ? CUT_NAME_ALIAS[opt.prodId]?.find((x) => x.match.test(nm)) : undefined;
  if (alias) return alias.as;
  // แผ่นมุง: ใบตัดเขียนกลาง ๆ ว่า "แผ่นหลังคา" (ชนิดอยู่ในดรอปดาว) · คิดราคาเรียกตามชนิดที่เลือก
  //   ไม่แปลงชื่อให้ตรงกัน = ขึ้น 2 แถว ("แผ่นไวนิล ใบตัด 0" + "แผ่นหลังคา คิดราคา 0") ทำเจ้าของงง
  //   ⚠ ฝาครอบไม่เข้าเงื่อนไขนี้ (ใบตัดไม่มีแถวฝาครอบ คิดราคาเดินคู่แผ่นเอง)
  const base = /^แผ่นหลังคา/.test(nm) && opt?.material
    ? `แผ่น${opt.material}`
    : (NO_CODE_PRICE.find((x) => x.match.test(nm))?.as ?? cleanRowName(nm));
  // รุ่นหลายด้านยุบข้ามด้านแล้ว → ต่อท้าย "(ทุกด้าน)" ให้ตรงกับชื่อฝั่งคิดราคา
  return opt?.prodId && !SINGLE_SIDE.has(opt.prodId) && ALU_FROM_CUTLIST[opt.prodId] ? allSides(base) : base;
}
...
const MH_W: Record<string, number> = {
  ...
  // เมทัลชีท 3 ชนิดตามไฟล์ v20.1 (3 ก.ย.69) — หน้ากว้างแถบ 34 ซม. เหมือน "เมทัลชีท" เดิม
  "เมทัลชีท EPS 2 นิ้ว เหล็ก": 34, "เมทัลชีท EPS 2 นิ้ว PVC": 34, "เมทัลชีท EPS 1 นิ้ว PVC": 34,
  "ชินโคร์ Shade 4มม": 138, "ชินโคร์ Prime 10มม": 138, "ชินโคร์ Nature 6มม": 138, "ชินโคร์ Grand 10มม": 138,
};
...
export function cutRoofConsumLines(
  ...
  for (const r of rows) {
    ...
    if (hit) {
      // ⚠ ของที่ขายเป็น "เส้น" (สต็อก 6 ม.) — ชิ้นที่ยาวเกิน 1 เส้น ต้องต่อ ใช้มากกว่า 1 เส้น
