...
export const PRODUCTS = {
  ...
  slimlux: {
    ...
    // รหัสอลูยึด "รหัสในสโตร์" (หมวด อลูมิเนียมเฟรมสลิม · เส้น 6 ม. · 4 สี)
    //   ของเดิมใช้รหัส WM-K ที่ไม่มีในสโตร์ → แก้ราคาในสโตร์แล้วราคาที่นี่ไม่ขยับ
    //   เจ้าของยืนยัน 20 ส.ค.69: "-40" ท้ายรหัส OPK ไม่ได้สื่ออะไร มีขนาดเดียว
    alu: [
      { name: 'ขวางบน/ล่าง', code: 'OPK-A201-40', price: 733, kg: 4.7, seg: 5, count: 'T.h' },
      { name: 'เสาบาน (ไส้เสา)', code: 'OPK-A202-40', price: 500, kg: 2.95, seg: 5, count: 'T.i' },
      { name: 'ตบเรียบ', code: 'OPK-A203-40', price: 343, kg: 1.57, seg: 5, count: 'T.j' },
      { name: 'ตบเกี่ยว', code: 'OPK-A204-40', price: 343, kg: 1.69, seg: 5, count: 'T.k' },
      // รางแขวน = เส้นอลู ไม่ใช่อุปกรณ์ (ของเดิมอยู่ในบล็อกอุปกรณ์ เลยผูกสโตร์ไม่ได้)
      { name: 'รางแขวน 1 ราง', code: 'XSW40008', price: 988, kg: 5.46, seg: 5, count: 'T.l' },
      { name: 'ฝาปิดล่าง เล็ก', code: 'XSW400013', price: 197, kg: 0.5, seg: 5, count: 'T.m' },
      { name: 'ฝาปิดล่าง ใหญ่', code: 'XSW400023', price: 233, kg: 0.8, seg: 5, count: 'T.n' },
      { box: 'ฉาก|2', name: 'ฉากปิดราง 2"', price: 320, kg: 1, seg: 'W', count: '2' },
      { name: 'กล่องเฟรม/คาน', price: 1400, kg: 5.98, seg: 5, count: 'T.p' },
      // บังใบ 4 หุน = กล่อง 4 หุน (เจ้าของยืนยัน 21 ส.ค.69) · ยาว = เสารับ − 3.6 ซม.
      { box: 'กล่อง|4หุน', name: 'บังใบ 4 หุน', price: 0, kg: 0, seg: 'H-0.061', count: '2' },
      // เสามือจับ X-J — ขายเป็นท่อนยาว 2.8 ม. · แยกรหัสตามสี (เจ้าของให้รหัส 21 ส.ค.69)
      //   ขาว JR02890 · ดำ JR02889 · สีอื่นใช้ "มิว" JR02891 (สีดิบ) แล้วบวกค่าอบตามน้ำหนัก
      //   น้ำหนัก 0.25 กก./ม. → ท่อนละ 0.7 กก. · ยาวตัด = เท่าเสากุญแจ (สูง − คาน 2.5 − 12.1 ซม.)
      //   เลือก "มือจับล็อค" → เป็น 0 (ใช้ร่วมกันไม่ได้)
      { name: 'เสามือจับ X-J', stockLen: 2.8,
        code: "CKEY==='white'?'JR02890':(CKEY==='black'?'JR02889':'JR02891')",
        price: 820, kg: 0.7, seg: 'H-0.146',
        count: "((spec&&spec.slxhandle)||'')==='X-J' ? (form==='เปิดคู่กลาง'?4:2) : 0" },
    ],
    glass: 'W*H',
    // ⚠ ลบ "9072 หัวบาน" + "9073 กลางบาน" ออก (20 ส.ค.69) — คิดซ้ำ
    //   ไฟล์น้ำหนักระบุ WM-K04 = "9072 หัวบาน (ขวางบน/ล่าง)" · WM-K01 = "9073 กลางบาน (เสากุญแจ)"
    //   = เส้นเดียวกับ ขวางบน/ล่าง กับ เสาบาน ที่คิดเป็นอลูอยู่แล้ว → เก็บเงิน 2 รอบ
    //   ชีต "คิดทุน SlimLux" ก็ไม่มี 2 บรรทัดนี้ (คิดเกิน ฿2,350 ที่ 200×200 2 บาน)
    ...
    //   ของจริงที่ต้องคิด = กล่องยาว/กล่องสั้น + ล้อล่าง + มือจับ + สักหลาด — ดึงจากใบตัดทั้งชุด
    hardware: [
      { name: 'ล้อล่าง (ตัว)', sku: 'JR00572', price: 100, unit: 'ตัว', count: 'T.v' },
      // มือจับล็อค แยกรหัสตามสี (JR00366 ขาว · JR00367 ดำ) ราคา 200 เท่ากัน
      { name: 'มือจับล็อค', sku: "((spec&&spec.slxhwcolor)==='ดำ')?'JR00367':'JR00366'", price: 200, unit: 'ชุด', count: 'T.s' },
      { name: 'สักหลาด', sku: 'JR00776', price: 1.5, unit: 'ม.', count: 'T.k*5' },
      // กล่องยาว/กล่องสั้น = ชุดล้อ+soft close+สลิง ที่ล็อกมาในกล่อง (เจ้าของยืนยัน 20-21 ส.ค.69)
      //   ของเดิมคิดราคาไม่คิดเงินกล่องเลย = แถมฟรี · ราคามาจากสโตร์
