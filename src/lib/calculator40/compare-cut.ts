...
export function statusOf(calc: number, cut: number, hasKey: boolean, orderOnly = false): AluRow["status"] {
  ...
  //   4 ก.ย.69: นับฝั่งใบตัดด้วย (เดิมเช็คแต่ calc>0) — ของสั่งตามงานที่มีแต่ในใบตัด เช่น อุปกรณ์บานยก
  //   ก็คือ "ไม่สต็อก" เหมือนกัน ไม่ใช่ "ยังไม่ผูกรหัส" (เจ้าของสั่ง "บานยกไม่ต้องผูกรหัส สั่งใหม่ทุกครั้ง")
  if (orderOnly && (calc > 0 || cut > 0)) return "ไม่สต็อก สั่งใหม่";
...
export function compareCut(PB: any, inp: CompareInput) {
  ...
  // ⚠⚠ เคยพังหนัก (เจ้าของจับได้ 2 ก.ย.69: "บานเฟี้ยม ทำไมยังขึ้นแบบเก่า และแถมเขียวอีก")
  //   ของเดิมทำแบบนี้:  calcHw = hwl?.length ? hwl.map(...) : engHw
  //   คือเอา "รายการจากใบตัด" (hwl) มาวางเป็นฝั่งคิดราคา แล้วเอาไปเทียบกับใบตัดอีกที
  //   = เทียบตัวเองกับตัวเอง → ขึ้น "ตรง" ทุกแถวเสมอ ไม่ว่าของจริงจะเป็นยังไง
  //   ร้ายกว่านั้น: รุ่นที่มีรหัสยังไม่ตั้งราคาแม้ตัวเดียว engine จะ "ถอย" ไปคิดเงินด้วยสูตรเก่า
  //   (ดู engine.mjs "กันคิดต่ำกว่าจริงเงียบ ๆ") แต่หน้าเทียบยังโชว์เขียว = หน้าจอโกหกเรื่องเงิน
  //
  //   กติกาใหม่: ฝั่งคิดราคา = "ของที่ engine คิดเงินจริง" เสมอ (engHw)
  //   ยกเว้นกรณีเดียว — engine ใช้รายการจากใบตัดจริง (calc.hwFromCutlist)
  ...
  for (const h of (cut?.hardware ?? [])) {
    ...
    if (e) { e.cutQty = r2(e.cutQty + h.qty); e.cutUnit = h.unit; }
      ...
      // ของสั่งตามงานในใบตัด (noStock ไม่มีรหัส) ต้องติดธงมาด้วย ไม่งั้นขึ้น "ไม่มีรหัส" ทั้งที่ตั้งใจไม่ผูก
      orderOnly: !!((h as { noStock?: boolean }).noStock && !h.sku),
