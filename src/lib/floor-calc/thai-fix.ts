...
const SURE: Rule[] = [
  ...
  // ไมโครไพล์ (micropile) — คำทับศัพท์ผิดบ่อยมาก (ช่างพิมพ์หลายแบบ) · คำที่ถูกอยู่แล้ว "ไมโครไพล์" ไม่แตะ
  //   ครอบ: ไฟล/ไฟย์ (ไมโครไฟล์/ไมโคไฟล์/ไมโครไฟย์) · ไพน/ไพว/ไพส (ไมโครไพน์…) · พาย (ไมโครพาย) ·
  //         ไมโคไพล์ (ตก ร) · ไมโครไพ ที่ไม่ตามด้วย "ล์" (ไมโครไพ/ไมโครไพย์)
  { from: /ไมโคร?ไฟล?ย?์?|ไมโคร?ไพน์?|ไมโคร?ไพว์?|ไมโคร?ไพส์?|ไมโคร?พาย|ไมโคไพล์|ไมโครไพ(?!ล์)ย?์?/g, to: "ไมโครไพล์", why: "ไมโครไพล์ (micropile)" },
...
export function fixThai(input: string, applySuggest = false): FixResult {
  ...
  const run = (rules: Rule[], sure: boolean, apply: boolean) => {
    for (const r of rules) {
      ...
      if (before === after) continue;   // คำที่ถูกอยู่แล้ว (แม็ปเป็นตัวเอง) → ไม่รายงานเป็น "คำที่จะแก้"
