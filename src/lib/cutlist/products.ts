...
function casementDoorHardware(hasSill: (o: CutInput) => boolean, sashN: (o: CutInput) => number = () => 1, opt: { fileSet?: "woodjamb" } = {}): HardwareDef[] {
  ...
  return [
    ...
    // 21 ก.ย.69: กลับมาใช้รหัส "สโตร์จริง" — JR00482/JR00557 มีแต่ในชีต ราคา ERP ของไฟล์ แต่ไม่มีในสโตร์
    //   (ตรวจย้อนหลัง 1 ก.ย.69 ⑧ + เจ้าของเคาะ 27 ส.ค.: สปิงก็อท = JR00592 · ฉากประคองมุม = JR00267 → รวมเป็น JR00480 เมื่อ 4 ก.ย.)
    //   ราคายังยึดไฟล์ (คิดทุน โซลิด/กระทุ้ง = 12 / 5) ผ่าน HWPRICE_BY_PROD — รหัสยึดสโตร์ เพื่อให้ตัดสต็อกได้จริง
    ...(wj ? [] : [
      { name: "สปิงก็อท", sku: "JR00592", qty: (o: CutInput) => 4 * sashN(o), unit: "ตัว" },
      { name: "ฉากประคองมุม", sku: "JR00480", qty: (o: CutInput) => 8 * sashN(o), unit: "ตัว" },
    ]),
    ...
    { name: "มือจับ Cmech ล็อค", sku: "JR00291", qty: (o) => handleN(o, "Cmech กุญแจ+ล็อค") + handleN(o, "Cmech ล็อค+ดัมมี่"), unit: "ชุด" },
    { name: "มือจับ Cmech ดัมมี่", sku: "JR00289", qty: (o) => handleN(o, "Cmech ล็อค+ดัมมี่") + 2 * handleN(o, "Cmech ดัมมี่+ดัมมี่"), unit: "ชุด" },
    otherHandleRow("motherHandle"),
    otherHandleRow("childHandle", { label: "มือจับใบลอง (อื่นๆ)", gate: (o) => childN(o) > 0 }),
    ...
    { name: "CDQ บานเปิด (บานลอง)", sku: "JR00596", qty: childN, unit: "ตัว", note: "= จำนวนบาน − 1 (บานเดี่ยว = 0)" },
    { name: "ปลายกลอน (บานลอง)", sku: "JR00598", qty: childN, unit: "ตัว", note: "= จำนวนบาน − 1 (บานเดี่ยว = 0)" },
...
/** อ่าน opt ที่ยังไม่ได้ประกาศใน CutInput (engine.ts อยู่นอกขอบเขตเวฟนี้) — รูปแบบเดียวกับ fIn()/otherHandleRow() */
const optStr = (o: CutInput, key: string, def: string) => String((o as unknown as Record<string, unknown>)[key] ?? def);
/** เลือกรหัสตามสี (ดำ = ค่าตั้งต้นของทุกชีต v1) */
const byColor = (key: string, dark: string, white: string, def = "ดำ") => (o: CutInput) => (optStr(o, key, def) === "ดำ" ? dark : white);
...
//   v1 (16 ก.ย.69): ทั้ง 3 ชีต SMS_ตัด_* แถว 6 เขียน "B20010 ผูกกับความสูง ≥240 ซม." → ใช้ >= ไม่ใช่ >
...
export const SMS_SLIDE_FREE: CutSpec = {
  ...
  profiles: [
    ...
    // v1: SMS_ตัด_อิสระ แถว 20 "ตบเฟรมบน/ล่าง ร่องใน" รหัส = B20019 (เดิมเว็บเว้น "-" → ไม่คิดเงิน/ไม่หักสต็อก)
    { name: "ตบเฟรมบน/ล่าง ร่องในบน", code: "B20019", len: (o) => o.W - 7, qty: (o) => (meshOf(o) === "ไม่มี" ? Math.max(3 - o.N, 0) : 0) },
    { name: "ตบเฟรมบน/ล่าง ร่องในล่าง", code: (o) => (meshOf(o) === "เฟรมเล็ก" ? "B20048" : "B20019"), len: (o) => o.W - 7, qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? 0 : Math.max(3 - o.N, 0)) },
    { name: "เบรคบาน (ธรณี)", code: "B20050", len: (o) => o.W - 4.4, qty: (o) => (o.rail === "รางเตี้ย7มม" ? 2 : 0) },
    // v1: SMS_ตัด (ชีตรวม) D20 = กว้างเต็ม (ไม่หัก 4.4) · E20 = F2 "บานเลื่อน" (อิสระ = N)
    { name: "ตบรางล้อ", code: "F7994", len: (o) => o.W, qty: (o) => (isPlug(o.rail) ? o.N : 0) },   // รางเตี้ยไม่ใช้ตบรางล้อ (ใช้ B20050 แทน)
    { name: "ตบรางล้อ (มุ้งใหญ่)", code: "F7994", len: (o) => o.W, qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? meshCountOf(o) : 0) },
...
export const SMS_SLIDE_CENTER: CutSpec = {
  ...
  profiles: [
    ...
    { name: "ฝาปิดเฟรมข้าง", code: "B20019", len: (o) => o.H - (isPlug(o.rail) ? 5 : 2.3), qty: () => 4 },
    // v1: SMS_ตัด_คู่กลาง แถว 21-22 ร่องใน/ร่องกลาง รหัส = B20019 ทั้งคู่
    { name: "ตบเฟรมบน/ล่าง ร่องในบน", code: "B20019", len: (o) => o.W - 7, qty: (o) => (meshOf(o) === "ไม่มี" ? 1 : 0) },
    { name: "ตบเฟรมบน/ล่าง ร่องในล่าง", code: (o) => (meshOf(o) === "เฟรมเล็ก" ? "B20048" : "B20019"), len: (o) => o.W - 7, qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? 0 : 1) },
    { name: "ตบเฟรมบน/ล่าง ร่องกลาง", code: "B20019", len: (o) => (o.W - 4.4) - 2 * (centerCross(o) + 9.7), qty: () => 2, note: "เฟรมบน − 2×(ขวางล่าง+9.7)" },
    { name: "เบรคบาน (ธรณี)", code: "B20050", len: (o) => o.W - 4.4, qty: (o) => (o.rail === "รางเตี้ย7มม" ? 2 : 0) },
    // v1: ยาว = กว้างเต็ม · จำนวน = F2 "บานเลื่อน" (เปิดคู่กลาง = N−2 · ที่ N=4 ได้ 2 เท่าเดิม)
    { name: "ตบรางล้อ", code: "F7994", len: (o) => o.W, qty: (o) => (isPlug(o.rail) ? Math.max(o.N - 2, 0) : 0) },   // รางเตี้ยไม่ใช้ตบรางล้อ
    { name: "ตบรางล้อ (มุ้งใหญ่)", code: "F7994", len: (o) => o.W, qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? 2 : 0) },
...
export const SMS_SLIDE_TOW: CutSpec = {
  ...
  profiles: [
    ...
    { name: "ฝาปิดเฟรมข้าง", code: "B20019", len: (o) => o.H - (isPlug(o.rail) ? 5 : 2.3), qty: () => 4 },
    // v1: SMS_ตัด_ลากจูง แถว 20-21 ร่องกลาง/ร่องใน รหัส = B20019 ทั้งคู่
    { name: "ตบร่องบานเลื่อน", code: "B20019", len: (o) => towCross(o) + (o.N <= 2 ? 4.3 : 4.7), qty: () => 2, note: "N≤2: ขวางบน+4.7−0.4 · N≥3: ขวางบน+4.7" },
    { name: "ตบร่องบานตาย (ร่องในบน)", code: "B20019", len: (o) => (o.N <= 2 ? o.W - 7 : (o.W - 4.4) - 1.3 - towCross(o) - 9.7 * (o.N - 2)), qty: (o) => (meshOf(o) === "ไม่มี" ? 1 : 0), note: "N≤2: W−7 · N≥3: (W−4.4)−1.3−ขวางบน−9.7×(N−2)" },
    { name: "ตบร่องบานตาย (ร่องในล่าง)", code: (o) => (meshOf(o) === "เฟรมเล็ก" ? "B20048" : "B20019"), len: (o) => (o.N <= 2 ? o.W - 7 : (o.W - 4.4) - 1.3 - towCross(o) - 9.7 * (o.N - 2)), qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? 0 : 1) },
    { name: "เบรคบาน (ธรณี)", code: "B20050", len: (o) => o.W - 4.4, qty: (o) => (o.rail === "รางเตี้ย7มม" ? 2 : 0) },
    // v1: ยาว = กว้างเต็ม · จำนวน = F2 "บานเลื่อน" (ลากจูง = N−1) — ของเดิม "N ดิบ" มาจากไฟล์ v2 เก่า
    { name: "ตบรางล้อ", code: "F7994", len: (o) => o.W, qty: (o) => (isPlug(o.rail) ? Math.max(o.N - 1, 0) : 0) },   // รางเตี้ยไม่ใช้ตบรางล้อ (ใช้ B20050 แทน)
    { name: "ตบรางล้อ (มุ้งใหญ่)", code: "F7994", len: (o) => o.W, qty: (o) => (meshOf(o) === "เฟรมใหญ่" ? meshCountOf(o) : 0) },
...
export const VELORA_SWING: CutSpec = {
  ...
  profiles: [
    ...
    // v1 Velora_ตัด G13/G14: รหัสตามสีลูกฟูก (ดำ JR01994 / ขาว JR01993) — คิดราคาใช้ JR01994 เป็นฐาน
    { name: "ลูกฟูก 2 ทาง แนวตั้ง", code: "JR01994", len: (o) => o.H + (vFit(o.rail) ? 0 : 1), qty: (o) => (vFit(o.rail) ? 2 : 0) },
    { name: "ลูกฟูก 2 ทาง แนวนอน", code: "JR01994", len: (o) => o.W + (vFit(o.rail) ? 0 : 2), qty: (o) => (vFit(o.rail) ? 1 : 0) },
  ],
  hardware: [
...
const ebSlide = (o: CutInput) => {
  ...
  return 2 * (Math.floor(L / 2) + Math.floor(R / 2));
};
/** มีบานคี่ไหม (P11) — ชนผนัง-คี่ · คี่+คี่ · คู่+คี่ = 1 · ชนผนัง-คู่ · คู่+คู่ = 0 */
const ebOdd = (o: CutInput) => {
  const L = Number(o.L ?? Math.ceil(o.N / 2)), R = o.N - L;
  const hi = Math.max(L, R), lo = Math.min(L, R);
  return hi % 2 === 1 || lo % 2 === 1 ? 1 : 0;
};
/** HD-305 = 2 เฉพาะกรณี "คี่+คี่" และ "คู่+คี่" (สองฝั่งจริง ไม่ใช่ชนผนัง) */
const ebHd305 = (o: CutInput) => {
  const L = Number(o.L ?? Math.ceil(o.N / 2)), R = o.N - L;
  const hi = Math.max(L, R), lo = Math.min(L, R);
  return lo > 0 && (hi % 2 === 1 || lo % 2 === 1) ? 2 : 0;
};
...
export const EURO_BIFOLD: CutSpec = {
  ...
  opts: [
    ...
    { key: "glass", label: "กระจก (มม.)", type: "number" },
    // ชีตตั้งต้น "สีฮาร์ดแวร์ ดำ" — ใช้เลือกรหัส HD-481A (JR02946 ดำ / JR02945 ขาว)
    { key: "hwColor", label: "สีฮาร์ดแวร์", choices: ["ดำ", "ขาว"] },
  ],
  defaults: { W: 249.5, H: 210, N: 4, L: 2, rail: "เฟรมล่าง", honk: false, glass: 6, hwColor: "ดำ" },
  profiles: [
  ...
  hardware: [
    ...
    // v1 เฟี้ยมยูโร_ตัด D55 = 2×$P$12 = 2×(INT(ซ้าย/2)+INT(ขวา/2)) — เดิมเว็บตรึง 2
    //   ตรวจ: 2-0 → 2 · 3-0 → 2 · 4-0 → 4 · 2-2 → 4
    { name: "HD-1180 ก้านสไลด์", sku: "HD-1180", qty: ebSlide, unit: "ตัว" },
    { name: "HD-213 ฉากเข้ามุม", sku: "HD-213", qty: (o) => 4 * o.N, unit: "ตัว" },
    { name: "HD-200 ฉากประคองมุม", sku: "HD-200", qty: (o) => 12 * o.N, unit: "ตัว" },
    // ── ชุดมัลติพ้อยท์ (เฉพาะ "บานคี่") — v1 เฟี้ยมยูโร_ตัด แถว 10-17 · ราคา ERP ยัง 0 ("ไม่สต็อค") ──
    //   P11 = 1 เมื่อฝั่งใดฝั่งหนึ่งเป็นบานคี่ (ชนผนัง-คี่ · คี่+คี่ · คู่+คี่) ไม่งั้น 0
    //   21 ก.ย.69 เจ้าของยืนยัน "รหัสพวกนี้ไม่มีจริงในสโตร์" → ตั้งเป็นของสั่งตามงาน (noStock)
    //   คงรหัสไว้เป็นข้อมูลอ้างอิงจากไฟล์ แต่ไม่ต้องไปหาในสโตร์/ไม่หักสต็อก · ราคาตามไฟล์ = 0 (HWPRICE_ZERO)
    { name: "HD-481A มัลติพ้อยท์", sku: byColor("hwColor", "JR02946", "JR02945"), qty: ebOdd, unit: "ชุด", noStock: true },
    { name: "HD-1115 มัลติพ้อยท์", sku: "JR02947", qty: ebOdd, unit: "ชุด", noStock: true },
    { name: "HD-850 มัลติพ้อยท์", sku: "JR02948", qty: ebOdd, unit: "ชุด", noStock: true },
    { name: "HD-803 มัลติพ้อยท์", sku: "JR02949", qty: ebOdd, unit: "ชุด", noStock: true },
    { name: "HD-305 มัลติพ้อยท์", sku: "JR02950", qty: ebHd305, unit: "ชุด", noStock: true, note: "คี่+คี่ / คู่+คี่ = 2 · อื่น 0" },
    { name: "จุดรับล็อค มัลติพ้อยท์", sku: "JR02951", qty: (o) => ebOdd(o) * (o.H <= 270 ? 3 : 4), unit: "จุด", noStock: true },
    { name: "หมุดล็อค มัลติพ้อยท์", sku: "JR02952", qty: (o) => ebOdd(o) * (o.H <= 270 ? 3 : 4), unit: "ตัว", noStock: true },
    { name: "ก้านสไลด์ มัลติพ้อยท์", sku: "JR02953", qty: (o) => ebOdd(o) * (Math.floor(euroSashH(o) / 10) / 10), unit: "ม.", noStock: true, note: "ROUNDDOWN(สูงบาน/100, 1) ม." },
...
export const FUJI_SLIDE: CutSpec = {
  ...
  profiles: [
    ...
    // v1: ยูโร_ตัด #25 "เสารับแรง (สูงเกิน 280)" F7951 ยาว = สูงช่อง (3000 มม.) × 1 ท่อน · เงื่อนไข สูง > 280 ซม.
    { name: "เสารับแรง", code: "F7951", len: (o) => o.H, qty: (o) => (o.H > 280 ? 1 : 0), note: "v1 ยูโร_ตัด #25 · สูง>280 → 1 ท่อน ยาว=สูงช่อง" },
    ...
    // v1: ยูโร_ตัด #21 "ตบกันสาด (ปิด)" รหัส F7992 (ไม่ใช่ F7988) · คงเงื่อนไขมุ้งของเว็บไว้
    { name: "ตบกันสาด (ปิด)", code: "F7992", len: (o) => o.W, qty: (o) => (fIn(o) ? (fMesh(o) ? 1 : 0) : 1) },
  ...
  hardware: [
    // เตือนบนใบตัด: สั่งมุ้งมาแต่เป็น 3 ราง — ไฟล์ตัดประกอบ FUJI มีชีตมุ้งเฉพาะ "เลื่อนสลับ 2 บาน"
    //   ห้ามเดาสูตรเอง แต่ต้องไม่หายเงียบ ๆ ให้ช่างเห็นว่ามีมุ้งที่ยังไม่มีรายการตัด
    ...
    // v1: ยูโร_ตัด #17 "มุมเลื่อน JR00480 = 8/บาน (ตามชีทคิดทุน)" — เดิมเว็บ 12/บาน (เจ้าของเคาะ 21 ส.ค.69)
    { name: "ฉากประกอบมุม", sku: "JR00480", qty: (o) => 8 * frc(o).p, unit: "ตัว" },
    ...
    // v1: สักหลาด D60 = (4·N·(ขวาง+เสา) + 8·เฟรมบน-ล่าง + เฟรมข้าง)/100  (ตรวจแล้ว 600×300 3ราง = 110.2 ม.)
    { name: "สักหลาด (ม.)", sku: "JR00794", unit: "ม.", noStock: true, note: "สะสมม้วน · สูตร v1 ยูโร_ตัด D60",
      qty: (o) => Math.round((4 * frc(o).p * (fSash(o) + fPost(o)) + 8 * (o.W - 4.2) + o.H) / 100 * 10) / 10 },
    ...fujiSlideConsum(),
  ],
};
...
function fujiSlideConsum(panels: (o: CutInput) => number = (o) => frc(o).p): HardwareDef[] {
  return [
    ...
    // v1: ยูโร_ตัด #12 "น็อต 6 หุน (ยึดล้อ) JR00863 = 4/บานเลื่อนจริง" (เดิมเว็บไม่มีแถวนี้)
    { name: "น็อต 6 หุน (ยึดล้อ)", sku: "JR00863", qty: (o) => 4 * panels(o), unit: "ตัว" },
    { name: "ซิลิโคน ใน+นอก", sku: "JR00504", qty: (o) => Math.ceil(((2 * (o.W + o.H)) / 100) * 2 / 12.5), unit: "หลอด" },
  ];
}
...
export const FUJI_SLIDE_CENTER: CutSpec = {
  ...
  profiles: [
    ...
    // v1 + decisions ข้อ 6: คู่กลางใส่ F7855 ×1 + F7925 ×1 + F7880 ×2 + F7879 ×2 "พร้อมกัน" (เลิก either/or ตามความสูง)
    { name: "ต่อชนกลาง", code: "F7925", len: fcPost, qty: () => 1, note: "v1 ยูโร_ตัด #29 · เปิดคู่กลาง 1 เส้น" },
    { name: "ชนกลาง", code: "F7855", len: fcPost, qty: () => 1, note: "v1 ยูโร_ตัด #26 · เปิดคู่กลาง 1 เส้น" },
    { name: "ตบปิดเฟรมร่องใน", code: "F7880", len: (o) => o.W - 4.2, qty: () => 2, note: "v1 ยูโร_ตัด #27 · ยาว = เฟรมบน-ล่าง" },
    { name: "ตบปิดเฟรมร่องกลาง", code: "F7879", len: (o) => (o.W - 4.2) / 2, qty: () => 2, note: "v1 ยูโร_ตัด #28 · ยาว = ครึ่งเฟรมบน-ล่าง" },
    ...
    // ตบกันสาด — งานใน (รางเตี้ย) ไม่ใช้ (เจ้าของเคาะ 20 ส.ค.69) · v1: "ตบกันสาด (ปิด)" = F7992 ไม่ใช่ F7988
    { name: "ตบกันสาด (ปิด)", code: "F7992", len: (o) => o.W, qty: (o) => (fIn(o) ? 0 : 1) },
  ...
  hardware: [
    ...
    ...handleHardware("LR"),
    { name: "สปิงก็อท", sku: "JR00592", qty: (o) => 4 * o.N, unit: "ตัว" },
    // v1: มุมเลื่อน JR00480 = 8/บาน (เดิม 12/บาน)
    { name: "ฉากประกอบมุม", sku: "JR00480", qty: (o) => 8 * o.N, unit: "ตัว" },
    { name: "ยางรูน้ำ", sku: "JR00589", qty: (o) => 2 + Math.max(0, Math.ceil((o.W - 4.2 - 150) / 50)), unit: "อัน" },
    { name: "วาวรูน้ำ", sku: "JR00485", qty: (o) => 2 + Math.max(0, Math.ceil((o.W - 4.2 - 150) / 50)), unit: "อัน" },
    // v1: สักหลาด = (4·N·(ขวาง+เสา) + 8·เฟรมบน-ล่าง + เฟรมข้าง)/100
    { name: "สักหลาด (ม.)", sku: "JR00794", unit: "ม.", noStock: true, note: "สะสมม้วน · สูตร v1 ยูโร_ตัด D60",
      qty: (o) => Math.round((4 * o.N * (fcSash(o) + fcPost(o)) + 8 * (o.W - 4.2) + o.H) / 100 * 10) / 10 },
...
export const FUJI_SLIDE_MULTI: CutSpec = {
  ...
  opts: [{ key: "work", label: "งาน", choices: ["ภายนอก", "ภายใน"] }, ...HANDLE_OPTS_LR],
  defaults: { W: 800, H: 240, N: 4, work: "ภายนอก", handleBrand: "Align", handleColor: "อบขาว", handleL: "กุญแจ+ล็อค", handleR: "ล็อค+ดัมมี่" },
  profiles: [
    ...
    // v1: ยูโร_ตัด ⑨ ต่อราง F7990 — งานนอก เลื่อน4/เลื่อน5 = 4 · งานใน 4ราง = 1 · 5ราง = 2
    //   ⚠ บล็อก "4ราง/5ราง ภายใน" ในไฟล์ดูเหมือนสลับคอลัมน์ (เฟรมล่างภายใน F7902 ×4/×5) — decisions ข้อ 6 สั่ง "ทำตามไฟล์ไปก่อน แล้วให้เจ้าของตรวจ"
    { name: "ต่อราง", code: "F7990", len: (o) => o.W - 4.2, qty: (o) => (fIn(o) ? (o.N === 5 ? 2 : 1) : fm(o).joinRail) },
    ...
    // v1 ยูโร_ตัด #25: เสารับแรง F7951 = 1 ท่อน ยาว = สูงช่อง เมื่อสูง > 280 (เหมือน FUJI_SLIDE/CENTER ที่เวฟ 2 แก้แล้ว)
    { name: "เสารับแรง", code: "F7951", len: (o) => o.H, qty: (o) => (o.H > 280 ? 1 : 0), note: "v1 ยูโร_ตัด #25 · สูง>280 → 1 ท่อน ยาวเท่าสูงช่อง" },
  ...
  hardware: [
    ...
    // v1: สักหลาด = (4·N·(ขวาง+เสา) + 8·เฟรมบน-ล่าง + เฟรมข้าง)/100
    { name: "สักหลาด (ม.)", sku: "JR00794", unit: "ม.", noStock: true, note: "สะสมม้วน · สูตร v1 ยูโร_ตัด D60",
      qty: (o) => Math.round((4 * o.N * (fmSash(o) + fmPost(o)) + 8 * (o.W - 4.2) + o.H) / 100 * 10) / 10 },
    ...fujiSlideConsum((o) => o.N),
  ],
};
...
// ⑪ FUJI ประตูเดี่ยว (dropdown เสา 10/8 ซม. · ธรณี มี/ไม่มี)
// v1 (บานเปิด_ตัด แผงแก้สูตร แถว 54-60 + ⑥ แถว 86):
//   ธรณี = F7938B (ไม่ใช่ F7938) · กรอบประตู = F7864 (ไม่ใช่ "7864" ที่ไม่มี F → ผูกสโตร์/PB ไม่ติด)
//   คิ้ว ตั้ง = เสา−80−80 · คิ้ว ขวาง = ขวาง−60−60 (หน่วยไฟล์ มม. → 16.0/12.0 ซม.) ทั้ง 2 ตัวเลือกเสา
//   ตบธรณี F7960 ยาว = กว้างเต็ม (D86 = C11) ไม่ใช่ W−5
//   ไม่มีธรณี → ใช้ เสริมใต้บาน F7863 ยาวเท่าธรณี (W−5) แทน
const fdSill = (o: CutInput) => o.sill !== "ไม่มีธรณี";
const fdPost = (o: CutInput) => (o.box === "8 cm · F7943B" ? "F7943B" : "F7864");
export const FUJI_DOOR: CutSpec = {
  id: "fuji_door", name: "FUJI ประตูเดี่ยว", stockLen: 640, rails: [],
  opts: [
    { key: "box", label: "เสา", choices: ["10 cm · F7864", "8 cm · F7943B"] },
    { key: "sill", label: "ธรณี", choices: ["มีธรณี", "ไม่มีธรณี"] },
    ...SWING_DOOR_OPTS,
  ],
  defaults: { W: 90, H: 210, N: 1, rail: "", honk: false, box: "10 cm · F7864", sill: "มีธรณี", ...SWING_DOOR_DEF },
  profiles: [
    { name: "เฟรมข้าง", code: "F7859", len: (o) => o.H, qty: () => 2 },
    { name: "เฟรม บน", code: "F7859", len: (o) => o.W - 5.0, qty: () => 1 },
    { name: "ธรณี", code: "F7938B", len: (o) => o.W - 5.0, qty: (o) => (fdSill(o) ? 1 : 0) },
    { name: "เสริมใต้บาน", code: "F7863", len: (o) => o.W - 5.0, qty: (o) => (fdSill(o) ? 0 : 1), note: "ใช้แทนธรณีเมื่อเลือก ไม่มีธรณี" },
    { name: "เสา", code: fdPost, len: (o) => o.H - 3.7, qty: () => 2 },
    { name: "ขวาง", code: fdPost, len: (o) => o.W - 3.7, qty: () => 2, note: "อลูเดียวกับเสา" },
    { name: "คิ้ว ตั้ง", code: "F7935", len: (o) => o.H - 3.7 - 16.0, qty: () => 2, note: "v1: เสา−80−80" },
    { name: "คิ้ว ขวาง", code: "F7935", len: (o) => o.W - 3.7 - 12.0, qty: () => 2, note: "v1: ขวาง−60−60" },
    { name: "ตบธรณี", code: "F7960", len: (o) => o.W, qty: (o) => (fdSill(o) ? 1 : 0), note: "v1: ยาว = กว้างเต็ม" },
  ],
  // ⑤ อุปกรณ์ FUJI ประตูเดี่ยว (SKU ชุดเดียวกับบานโซลิด · มีธรณี → น็อต 8 · ไม่มี → 6)
  hardware: casementDoorHardware(fdSill),
};

// ⑪b FUJI ประตูคู่ (บานเปิดคู่_ตัด) — 2 บาน + เปิดกลาง F7945C
// v1 แผงแก้สูตร แถว 54-61 (ที่ 1500×2000): วงกบข้าง 2000×2 · วงกบบน/ธรณี 1450×1 · กรอบตั้ง 1963×4
//   กรอบขวาง = (กว้าง−18.5−2−18.5)/2 = 730.5 · คิ้วตั้ง = กรอบตั้ง−80−80 · คิ้วขวาง = กรอบขวาง−60−60
//   เปิดกลาง F7945C = สูง−27−27 ×1
// ⚠ ไฟล์ขัดกันเอง: กรอบขวาง L58 = 2 ท่อน แต่ คิ้วขวาง L60 = 4 — 2 บานต้องมีขวาง 4
//   decisions ข้อ 7 เคาะ "ใช้ ×4" แล้วให้เจ้าของแก้ไฟล์
export const FUJI_DOOR_DOUBLE: CutSpec = {
  id: "fuji_door_double", name: "FUJI ประตูคู่ (2 บาน)", stockLen: 640, rails: [],
  opts: [
    { key: "box", label: "เสา", choices: ["10 cm · F7864", "8 cm · F7943B"] },
    { key: "sill", label: "ธรณี", choices: ["มีธรณี", "ไม่มีธรณี"] },
    ...SWING_DOOR_OPTS,
    { key: "childHandle", label: "มือจับ ใบลอง", choices: ["ไม่ใส่", "คิงโบ ล็อค+กุญแจ", "คิงโบ ดัมมี่+ดัมมี่", "Cmech กุญแจ+ล็อค", "Cmech ล็อค+ดัมมี่", "Cmech ดัมมี่+ดัมมี่", "อื่นๆ"] },
  ],
  defaults: { W: 150, H: 200, N: 2, rail: "", honk: false, box: "10 cm · F7864", sill: "มีธรณี", ...SWING_DOOR_DEF, childHandle: "ไม่ใส่" },
  profiles: [
    { name: "วงกบ ข้าง", code: "F7859", len: (o) => o.H, qty: () => 2 },
    { name: "วงกบ บน", code: "F7859", len: (o) => o.W - 5.0, qty: () => 1 },
    { name: "ธรณี", code: "F7938B", len: (o) => o.W - 5.0, qty: (o) => (fdSill(o) ? 1 : 0) },
    { name: "เสริมใต้บาน", code: "F7863", len: (o) => o.W - 5.0, qty: (o) => (fdSill(o) ? 0 : 1) },
    { name: "กรอบประตู ตั้ง", code: fdPost, len: (o) => o.H - 3.7, qty: () => 4 },
    { name: "กรอบประตู ขวาง", code: fdPost, len: (o) => (o.W - 3.9) / 2, qty: () => 4, note: "v1: (กว้าง−18.5−2−18.5)/2 · จำนวน ×4 ตาม decisions ข้อ 7" },
    { name: "คิ้ว ตั้ง", code: "F7935", len: (o) => o.H - 3.7 - 16.0, qty: () => 4 },
    { name: "คิ้ว ขวาง", code: "F7935", len: (o) => (o.W - 3.9) / 2 - 12.0, qty: () => 4 },
    { name: "เปิดกลาง", code: "F7945C", len: (o) => o.H - 5.4, qty: () => 1, note: "v1: สูง−27−27" },
    { name: "ตบธรณี", code: "F7960", len: (o) => o.W, qty: (o) => (fdSill(o) ? 1 : 0) },
  ],
  hardware: casementDoorHardware(fdSill, () => 2),
};
...
export const PC_DOOR: CutSpec = {
  ...
  opts: [
    ...
    { key: "sill", label: "ธรณี", choices: ["มีธรณี", "ไม่มีธรณี"] },
    // v1 PCDoor_ตัด: E66 "สีอลูเมืองทอง" · C65 "ฝาครอบรางบน สี" · C64 "ด้านหัวต่อราง" (ค่าตั้งต้น ดำ/ซ้าย)
    { key: "mtongColor", label: "สีอลูเมืองทอง", choices: ["ดำ", "ขาว"] },
    { key: "capColor", label: "สีฝาครอบรางบน", choices: ["ดำ", "ขาว"] },
    { key: "railSide", label: "หัวต่อราง ด้าน", choices: ["ซ้าย", "ขวา"] },
    ...HANDLE_OPTS_LR,
  ],
  defaults: { W: 300, H: 240, N: 2, rail: "", honk: false, beam: "1×4", split: "แบ่ง 2", sill: "มีธรณี", mtongColor: "ดำ", capColor: "ดำ", railSide: "ซ้าย", handleBrand: "Align", handleColor: "อบขาว", handleL: "กุญแจ+ล็อค", handleR: "ล็อค+ดัมมี่" },
  profiles: [
    { name: "คาน", code: (o) => boxCode(o.beam ?? "1×4"), len: (o) => o.W, qty: () => 1, note: "ตัดเท่าช่อง" },
    // v1 PCDoor_ตัด แถว 16-20 — ไฟล์ใส่รหัสเมืองทองมาครบแล้ว
    { name: "ฝาครอบรางบน", code: byColor("capColor", "JR03135", "JR03136"), len: (o) => o.W - 3.3 - 2.5, qty: () => 1 },
    { name: "รางบนบานเลื่อน", code: "JR03141", len: (o) => o.W - 4.5 - 2.5, qty: () => 1 },
    // v1 + decisions ข้อ 8: รื้อบานเปิดเป็นโปรไฟล์เมืองทอง JR031xx ทั้งชุด (เดิม F7859/F7864 ของ v20.1)
    { name: "วงกบบานเปิด", code: byColor("mtongColor", "JR03129", "JR03130"), len: (o) => o.H - pcBeamCut(o), qty: () => 1 },
    { name: "เสารับบานเลื่อน", code: boxCode("1×4"), len: (o) => o.H - pcBeamCut(o), qty: () => 1, note: "v1: กล่อง 1×4 (JR01841/40 ตามสี)" },
    { name: "ชนกลางรับบานเลื่อน", code: byColor("mtongColor", "JR03125", "JR03128"), len: (o) => o.H - pcBeamCut(o) - 4, qty: () => 1 },
    ...
    { name: "กรอบบานเปิด เมืองทอง (สูง)", code: byColor("mtongColor", "JR03126", "JR03127"), len: (o) => o.H - pcBeamCut(o) - (pcNoSill(o) ? 3 : 6.3), qty: pcN },
    // v1 + decisions ข้อ 8 "ระยะหักตามไฟล์ v1": กว้างเปิด −0.7 (แบ่ง2) / +0.5 (แบ่ง4)
    //   (กระดาน G38/H38 · เว็บเคยพอร์ต −1.9 จากไฟล์ 24 ส.ค. — v1 กลับมาเป็น −0.7)
    { name: "กรอบบานเปิด เมืองทอง (กว้าง)", code: byColor("mtongColor", "JR03126", "JR03127"), len: (o) => o.W / pcN(o) + (o.split === "แบ่ง 4" ? 0.5 : -0.7), qty: pcN },
    ...
    // v1 PCDoor_ตัด แถว 24-25: เสากุญแจ 1 เส้น/บานเลื่อน (= C12/2) + เพิ่ม "เสาเกี่ยว B20009" ยาวเท่ากัน
    //   (เว็บเดิมใช้ pcN ทั้งก้อน = 2 เท่า · คอมเมนต์เก่าเขียนเองว่า "เจ้าของจะแก้อีกรอบ รอไฟล์" — v1 คือไฟล์ที่รอ)
    { name: "กรอบบานเลื่อน sms (สูง)", code: "B20051", len: (o) => o.H - pcBeamCut(o) - (pcNoSill(o) ? 5.8 : 10.3), qty: (o) => pcN(o) / 2 },
    { name: "เสาเกี่ยว (บานเลื่อน)", code: "B20009", len: (o) => o.H - pcBeamCut(o) - (pcNoSill(o) ? 5.8 : 10.3), qty: (o) => pcN(o) / 2 },
    // v1: กว้างเลื่อน −11.4 (แบ่ง2) / −10 (แบ่ง4) — เดิมเว็บ −10.2
    { name: "กรอบบานเลื่อน sms (กว้าง)", code: "B20054", len: (o) => o.W / pcN(o) - (o.split === "แบ่ง 4" ? 10 : 11.4), qty: pcN },
    // ── v1 + decisions ข้อ 8: "ตัดธรณี F7938B / ตบธรณี F7960 / F7945C ออก" ──
    //   ชีต v1 ไม่มี 3 แถวนี้ทั้งใบตัดและคิดทุน · ช่อง "ธรณี" เหลือผลแค่ระยะหัก + น็อตเฟรม 8/6
    //   แบ่ง 4 ใช้ "ชนกลางบานเลื่อน B20046" แทน F7945C
  ...
  hardware: [
    ...
    // v1 PCDoor_ตัด ⑤.1: หัวต่อราง = แถวเดียว เลือกรหัสตามด้าน (ซ้าย JR02969 · ขวา JR02968)
    //   เดิมเว็บออก 2 แถว (นับเกิน 1 ชิ้น) และรหัสซ้าย/ขวาสลับกับไฟล์
    { name: "หัวต่อราง", sku: (o) => (optStr(o, "railSide", "ซ้าย") === "ขวา" ? "JR02968" : "JR02969"), qty: (o) => pcN(o) / 2, unit: "อัน", note: "1/บานเลื่อน · ซ้าย=JR02969 ตามไฟล์ v1" },
    // ถอดแถวอุปกรณ์ "ฝาครอบราง" ออก — ซ้ำกับโปรไฟล์ "ฝาครอบรางบน" ที่นับเป็นเส้นอยู่แล้ว (ชีตนับครั้งเดียว)
    //   และรหัสเดิม JR03056/57 ไม่ตรงไฟล์ v1 (= JR03136/JR03135)
...
export const SOLID_DOOR: CutSpec = {
  ...
  opts: [
    ...
    { key: "corrColor", label: "สีลูกฟูก+เส้นคาด", choices: ["ดำ", "ขาว"] },
  ],
  defaults: { W: 120, H: 279, N: 2, rail: "", honk: false, sill: "มี", doorSplit: "แม่-ลูก", motherW: 80, hwColor: "ขาว", lockType: "ล็อคปกติ", openDir: "เปิดออก", motherHandle: "คิงโบ ล็อค+กุญแจ", childHandle: "ไม่ใส่", corrColor: "ดำ", solidLayer: "โซลิด 2 ชั้น" },
  profiles: [
    ...
    // v1 + decisions ข้อ 9: ลูกฟูก/เส้นคาด ใช้รหัสไฟล์ (คิดทุน บานโซลิด A32 + ราคา ERP แถว 144-147)
    //   สลับตามช่อง "สีลูกฟูก+เส้นคาด" (F9 · ค่าตั้งต้น ดำ) — เดิม code "-" → ไม่คิดเงิน/ไม่หักสต็อก (หาย ~4,132 บาท/ชุด)
  ...
  hardware: [
    ...
    // 21 ก.ย.69: ไฟล์ (โซลิด_ตัด ⑤.1 แถว 63-64) เขียน JR00482/JR00557 แต่ "สโตร์ไม่มี 2 รหัสนี้"

    //   → ใช้รหัสสโตร์ JR00592 (สปิงก็อท) · JR00480 (ฉากประกอบมุม) · ราคายังตามไฟล์ 12/5 (HWPRICE_BY_PROD)
    { name: "สปิงก็อท", sku: "JR00592", qty: (o) => 4 * o.N, unit: "ตัว" },
    { name: "ฉากประคองมุม", sku: "JR00480", qty: (o) => 8 * o.N, unit: "ตัว" },
    ...
    { name: "ยางกรอบบาน", sku: "JR00771", unit: "เมตร",
      // v1 โซลิด_ตัด D74 = ROUND((2×(บานแม่+สูง) + 2×(บานลูก+สูง)×จำนวนลูก)/100, 1)
      //   ของเดิมปัดเป็นเมตรเต็ม "ก่อน" คูณจำนวนบาน → เกินจริงทุกครั้งที่เศษเกินครึ่งเมตร
...
// ⑱ บานเปิดครอบวงกบไม้ (ครอบวงกบไม้_ตัด v1) — กล่องเรียบ/บังใบล้วน · N ∈ {1,2}
// ⚠ ไฟล์ไม่มีคอลัมน์เส้นสต็อก → ใช้ 600 (รอเจ้าของยืนยัน)
// v1 + decisions ข้อ 15: กล่องครอบวงกบ = **กล่อง 1.6"×4" เมืองทอง** (ไม่ใช่ 4"×4") ดึงราคาตามสีจากสโตร์
//   รหัสตามชีต คอลัมน์ F (สลับตามช่อง "สีกล่องอลู (รหัส)" F10 · ค่าตั้งต้น ดำ):
//     กล่องเรียบ 1.6"×4"  JR01985 ดำ / JR01984 อบขาว
//     บังใบกล่อง ½"×1"    JR01823 / JR01822
//     วงกบ-ธรณี มีติ่ง    JR03129 / JR03130
//     กรอบบาน 3" มีบังใบ  JR03131 / JR03132
//     กรอบบาน 3" ไม่บังใบ JR03126 / JR03127
...
const wCode = (dark: string, white: string) => byColor("boxColor", dark, white);
export const WOODJAMB_SWING: CutSpec = {
  ...
  opts: [
    ...
    { key: "motherW", label: "บานแม่ กว้าง (ซม.)", type: "number" },
    { key: "sill", label: "ธรณี", choices: ["มีธรณี", "ไม่มีธรณี"] },
    { key: "boxColor", label: "สีกล่องอลู (รหัส)", choices: ["ดำ", "อบขาว"] },
    ...SWING_DOOR_OPTS,
    { key: "childHandle", label: "มือจับ ใบลอง", choices: ["ไม่ใส่", "คิงโบ ล็อค+กุญแจ", "คิงโบ ดัมมี่+ดัมมี่", "Cmech กุญแจ+ล็อค", "Cmech ล็อค+ดัมมี่", "Cmech ดัมมี่+ดัมมี่", "อื่นๆ"] },
  ],
  defaults: { W: 130, H: 210, N: 2, rail: "", honk: false, doorSplit: "แม่ลูก", motherW: 80, sill: "มีธรณี", boxColor: "ดำ", ...SWING_DOOR_DEF, childHandle: "ไม่ใส่" },
  profiles: [
    { name: 'กล่องเรียบ 1.6"x4" แนวตั้ง (ครอบข้าง)', code: wCode("JR01985", "JR01984"), len: (o) => o.H - 4.3, qty: () => 2 },
    { name: 'กล่องเรียบ 1.6"x4" แนวนอน (ครอบบน)', code: wCode("JR01985", "JR01984"), len: (o) => o.W - 0.7, qty: () => 1 },
    { name: "บังใบกล่อง แนวนอน (บน)", code: wCode("JR01823", "JR01822"), len: (o) => o.W - 0.4, qty: () => 1, note: "45° 2ฝั่ง" },
    { name: "บังใบกล่อง แนวตั้ง (ข้าง)", code: wCode("JR01823", "JR01822"), len: (o) => o.H - 0.2 - (wSill(o) ? 4.5 : 0), qty: () => 2, note: "45° 1ฝั่ง" },
    { name: "ธรณี", code: wCode("JR03129", "JR03130"), len: (o) => o.W - 0.4, qty: (o) => (wSill(o) ? 1 : 0) },
    { name: "กรอบบานบังใบ แนวตั้ง", code: wCode("JR03131", "JR03132"), len: (o) => o.H - 0.2 - 0.8 + 2.7 - (wSill(o) ? 3.2 : 0), qty: (o) => o.N },
    { name: "กรอบบานไม่บังใบ แนวตั้ง", code: wCode("JR03126", "JR03127"), len: (o) => o.H - 0.2 - 0.8 - (wSill(o) ? 3.2 : 0), qty: (o) => o.N },
    { name: "กรอบบานบังใบ แนวนอน — บาน1", code: wCode("JR03131", "JR03132"), len: (o) => wDoor1(o) - 0.8 + 2.7, qty: () => 1 },
    { name: "กรอบบานบังใบ แนวนอน — บาน2", code: wCode("JR03131", "JR03132"), len: (o) => wDoor2(o) - 0.8 + 2.7, qty: (o) => (o.N === 2 ? 1 : 0) },
    { name: "กรอบบานไม่บังใบ แนวนอน — บาน1", code: wCode("JR03126", "JR03127"), len: (o) => wDoor1(o) - 0.8 - 3.2, qty: () => 1 },
...
// v1 หลังคา_ตัด E9 = ROUND(√(ยื่น² + ตกสโลป²),1) — ไฟล์กรอก "ตก สโลป (ซม.)" ไม่ใช่องศา (B8 · ค่าตั้งต้น 18)
const aRake = (o: CutInput) => r1(Math.sqrt((o.P ?? 0) ** 2 + (o.drop ?? 0) ** 2));
// ยาว จันทันรัดรอบ ฝั่งบ้าน / แป / ฝั่งปลาย(ไม่ยื่น) = (กว้าง − จันทันรวม×4.5) ÷ ช่อง  (C15)
const aPurLen = (o: CutInput) => (o.W - aNr(o) * 4.5) / aBays(o);
...
// แผง ⑦ v1 (F36–F38): ซอย − ยื่นปลาย 14.5 · ปิดปลาย 4 (เดิมเว็บ 2.5) · รางน้ำ 10.2
const aEndJack = (o: CutInput) => (o.roofEnd === "ยื่นปลาย" ? 14.5 : o.roofEnd === "ปิดปลาย" ? 4 : 10.2);
...
export const AWNING: CutSpec = {
  ...
  opts: [
    ...
    { key: "drop", label: "ตก สโลป (ซม.)", type: "number" },
  ...
  defaults: { W: 300, H: 0, N: 1, rail: "", honk: false, sheet: "ไวนิล", P: 150, drop: 18, purlin: "แปคู่", roofEnd: "รางน้ำ", rakeTotal: 0 },
  profiles: [
    // ── รื้อทั้งชุดตามไฟล์ v1 ชีต "หลังคา_ตัด" แถว 12–26 + แผงค่าหัก ⑦ (เวฟ 7) ──
    { name: "จันทันรัดรอบ ฝั่งบ้าน (ยัดในช่อง)", code: boxCode("1.6×4"), len: aPurLen, qty: aBays, note: "ยัดในช่องจันทัน · ยาวเท่าแป (เดิมเว็บใช้ ยาว=กว้าง−0.4 จำนวน 2 ท่อน)" },
    { name: "จันทันรัดรอบ (ยื่น ข้าง)", code: boxCode("1.6×4"), len: (o) => r1(aRake(o) - aEndSide(o)), qty: () => 2, note: "ปิด=เต็ม / ยื่น−10 / ราง−10.2" },
    { name: "จันทันซอย 1.6×4", code: boxCode("1.6×4"), len: (o) => r1(aRake(o) - aEndJack(o)), qty: (o) => Math.max(0, aNr(o) - 2), note: "ซอย = จันทันรวม − 2 (หักข้างซ้ายขวา)" },
    { name: "แป (ยัดในช่อง)", code: (o) => boxCode(o.purlin === "แปเดี่ยว" ? "1.6×1.6" : "1×1.5"), len: aPurLen, qty: (o) => aBays(o) * aNp(o) * dblP(o), note: "แปเดี่ยว=กล่อง1.6×1.6 · แปคู่=กล่อง1×1½" },
    { name: "ฉาก 6 หุน", code: ANGLE_6, len: (o) => o.W, qty: (o) => ceil(o.W / 600), note: "เต็มกว้าง" },
    { name: 'แซด 4"', code: ZBAR_4, len: (o) => o.W, qty: (o) => ceil(o.W / 600), note: "เต็มกว้าง" },
    { name: "จันทันรัดรอบ ฝั่งปลาย", code: boxCode("1.6×4"), len: (o) => (aOut(o) ? o.W - 0.4 : aPurLen(o)), qty: (o) => (aOut(o) ? 1 : aBays(o)), note: "ยื่นปลาย=เต็มกว้าง−0.4 · ปิด/ราง=ยัดในช่อง" },
    { name: "เพลทเหล็ก (กล่อง 1×1)", code: "-", len: (o) => r1((aRake(o) / 3) * 2.5), qty: aNr, note: "1 จุด/จันทัน · ตัดจริง ⅓ จันทัน ซ้อน 2 ท่อน + ดามล่าง = 2.5 ท่อน" },
    { name: "กล่องครอบเพลท 1.6×4", code: boxCode("1.6×4"), len: (o) => r1(aRake(o) * 0.25), qty: aNr, note: "25% ของยื่นเอียง" },
    { name: "รัดรอบ (หน้า)", code: boxCode("1×1.5"), len: (o) => (aOut(o) ? 0 : o.W + (o.roofEnd === "ปิดปลาย" ? 3.5 : 5.4)), qty: (o) => (aOut(o) ? 0 : 1), note: "ยื่นปลายไม่มีรัดรอบ" },
    { name: "รัดรอบ (ข้าง)", code: boxCode("1×1.5"), len: (o) => (aOut(o) ? 0 : r1(aRake(o) + (o.roofEnd === "ปิดปลาย" ? 0.1 : 2.7))), qty: (o) => (aOut(o) ? 0 : 2) },
    { name: "รางน้ำอลู", code: "-", len: (o) => o.W, qty: (o) => (o.roofEnd === "รางน้ำ" ? ceil(o.W / 600) : 0) },
    { name: "แผ่นหลังคา", code: "-", len: (o) => r1(aRake(o) - 3), qty: (o) => ceil(o.W / sW(o)), note: "ระยะตก − 3" },
    { name: 'กล่องปิดปลาย 1"×4"', code: boxCode("1×4"), len: (o) => o.W - 0.3, qty: (o) => (o.roofEnd === "ปิดปลาย" ? 1 : 0), note: "เฉพาะปิดปลาย · ⏳ คิดราคายังไม่มีบรรทัดนี้" },
    { name: "ฝาครอบ", code: "-", len: (o) => r1(aRake(o) - 3), qty: (o) => (o.sheet === "ไวนิล" || o.sheet === "โพลีตัน" ? ceil(o.W / sW(o)) : 0), note: "ยาวเท่าแผ่นหลังคา · เฉพาะไวนิล/โพลีตัน" },
  ],
};
...
// v1 จั่ว_ตัด C15: จันทันเฉียงตัดเต็มความเฉียง ไม่มีค่าหักปลาย (คิดราคา CUTEND = 0)
const gCutEnd = (_o: CutInput) => 0;
// แถวแป/ด้าน = ⌈เฉียง ÷ 50⌉ + 1 (จั่ว_ตัด E7 · เดิมเว็บใช้ครึ่งกว้างแทนความเฉียง)
const gPurRows = (o: CutInput) => ceil(gSlope(o) / 50) + 1;
// ยาวแป = (ลึก − จันทัน/ด้าน × 4.5) ÷ ช่อง (จั่ว_ตัด C16 · เดิมเว็บใช้ระยะห่างจันทัน − 4.5)
const gPurLen = (o: CutInput) => ((o.D ?? 0) - gN(o) * 4.5) / Math.max(1, gN(o) - 1);
export const GABLE_STRAIGHT: CutSpec = {
  ...
  defaults: { W: 400, H: 0, N: 1, rail: "", honk: false, sheet: "ไวนิล", D: 300, ridgeH: 60, purlin: "แปเดี่ยว", roofEnd: "รางน้ำ" },
  profiles: [
    // ── รื้อทั้งชุดตามไฟล์ v1 ชีต "จั่ว_ตัด" แถว 13–23 (เวฟ 7) ──
    { name: "จันทันสันจั่ว (ridge)", code: boxCode("1.6×4"), len: (o) => o.D ?? 0, qty: () => 1, note: "ยาว = ลึก D (เดิมเว็บใช้กล่อง 4×4 หั่นเป็นหลายท่อน)" },
    { name: "จันทันชายคา (2 ด้าน)", code: boxCode("1.6×4"), len: (o) => o.D ?? 0, qty: () => 2, note: "ยาว = ลึก D" },
    { name: "จันทันเฉียง 1.6×4 (2 ด้าน)", code: boxCode("1.6×4"), len: (o) => r1(gSlope(o) - gCutEnd(o)), qty: (o) => 2 * gN(o), note: "ยาว = เฉียง" },
    { name: "แป (ยัดในช่อง)", code: (o) => boxCode(o.purlin === "แปเดี่ยว" ? "1.6×1.6" : "1×1.5"), len: (o) => r1(gPurLen(o)), qty: (o) => (gN(o) - 1) * gPurRows(o) * 2 * dblP(o), note: "แปเดี่ยว=กล่อง1.6×1.6 · แปคู่=กล่อง1×1½ (จั่ว_ตัด G16)" },
    { name: "ฉาก 6 หุน (2 ชายคา)", code: ANGLE_6, len: (o) => o.D ?? 0, qty: (o) => 2 * ceil((o.D ?? 0) / 600), note: "ยาว = ลึก · ฝั่งคิดราคานับเป็นเส้น จึงตั้งชื่อต่างกันไม่ให้หน้าเทียบจับคู่ข้ามหน่วย" },
    { name: 'คาน 4"×4" T (เสารับสัน)', code: boxCode("4×4"), len: (o) => o.W + (o.ridgeH ?? 0), qty: (o) => ceil(gN(o) / 2), note: "ยาว = กว้าง + สูงสัน · เว้นจันทัน (เดิมเว็บแยกคานนอน/เสาตั้ง 2 แถว)" },
    { name: "รางน้ำอลู (2 ชายคา)", code: "-", len: (o) => o.D ?? 0, qty: (o) => (o.roofEnd === "รางน้ำ" ? 2 * ceil((o.D ?? 0) / 600) : 0) },
    { name: "แผ่นหลังคา (2 ด้าน)", code: "-", len: (o) => r1(gSlope(o) + 7), qty: (o) => 2 * ceil((o.D ?? 0) / sW(o)), note: "จันทันเต็ม + 7" },
    { name: "ฝาครอบ (2 ด้าน)", code: "-", len: (o) => r1(gSlope(o) + 7), qty: (o) => (o.sheet === "ไวนิล" || o.sheet === "โพลีตัน" ? 2 * ceil((o.D ?? 0) / sW(o)) : 0), note: "ยาวเท่าแผ่นหลังคา · เฉพาะไวนิล/โพลีตัน" },
    { name: 'กล่องเหล็ก 1"×1" (ยัดจันทันเฉียง)', code: "-", len: gSlope, qty: (o) => 2 * gN(o), note: "ไม่สต็อค · ฝั่งคิดราคานับเป็นเส้น (เศษส่วน×1.3) คนละหน่วยกับท่อน" },
    { name: "เพลทเหล็ก", code: "-", len: () => 0, qty: (o) => 4 * gN(o), note: "4 แผ่น × จันทัน/ด้าน · ไม่สต็อค" },
  ],
};
...
export const TOPRAIL_FRAME: CutSpec = {
  ...
  opts: [
    ...
    { key: "mtongColor", label: "สีอลูเมืองทอง", choices: ["ดำ", "อบขาว"] },
    ...HANDLE_OPTS_LR,
  ],
  defaults: { W: 360, H: 240, N: 2, rail: "", honk: false, sys: "SMS", sashMode: "อิสระ", fit: "ยัดในช่อง", handle: "ฝัง", beam: "2×4", mtongColor: "ดำ", handleBrand: "Align", handleColor: "อบขาว", handleL: "กุญแจ+ล็อค", handleR: "ล็อค+ดัมมี่" },
  profiles: [
    ...
    // v1 (รางบน_ตัด แถว 25-34 + ตาราง BF:BI) — ไฟล์ผูกรหัสครบแล้ว สลับตามช่อง "สีอลูเมืองทอง" (E12 · ค่าตั้งต้น ดำ)
    { name: "เสารับบาน (กล่อง)", code: byColor("mtongColor", "JR01841", "JR01840"), len: (o) => o.H - trBeam(o), qty: (o) => (trOut(o) === 1 ? 1 : 2), note: "v1: กล่อง 1×4 (C12 ค่าตั้งต้น)" },
    { name: "ชนกลางรับบาน", code: byColor("mtongColor", "JR03125", "JR03128"), len: (o) => o.H - trBeam(o), qty: () => 1, note: "ไฟล์ตั้ง 1 ตายตัว" },
    { name: "รางบน Hafele", code: "JR03141", len: (o) => (o.fit === "ยัดในช่อง" ? o.W - 5 : o.W * 2), qty: trSlide, note: "⚠ แปะนอก = W×2 (ไฟล์ตั้งใจว่า 2 ท่อนเท่า W)" },
    { name: 'ฉาก 4" ปิดราง', code: byColor("mtongColor", "JR01949", "JR01948"), len: (o) => o.W, qty: () => 2 },
    ...
    // v1: รางบน_ตัด แถว 32-34 ใส่รหัสยูโรแล้ว (F7980 ทั้งเสาตั้ง/นอน · F7983 ตบเกี่ยว)
    { name: "เสากุญแจยูโร (ตั้ง)", code: "F7980", len: (o) => o.H - trBeam(o) - 5.1, qty: (o) => (trSMS(o) ? 0 : trLock(o)) },
    { name: "เสากุญแจยูโร (นอน 45°)", code: "F7980", len: (o) => (o.W - trCeuro(o) + trOv(o) * 8) / o.N, qty: (o) => (trSMS(o) ? 0 : 2 * o.N) },
    { name: "ตบเกี่ยวยูโร", code: "F7983", len: (o) => o.H - trBeam(o) - 5.1, qty: (o) => (trSMS(o) ? 0 : trHook(o)) },
  ],
  ...
  hardware: [
    ...
    // v1 (16 ก.ย.69): รางบน_ตัด ⑤.1 แถว 85-96 **ไม่มีแถว "ไกด์ดำ"** → ถอดออกตามกฎ "อย่ากำหนดจำนวนเอง"
    //   (เดิมเว็บใส่ JR00558 ×2/×1 เอง · JR00558 ยังไม่มีราคาสโตร์ด้วย → เป็นตัวบล็อก HW_FROM_CUTLIST มาตลอด)
  ],
};
...
export const EURO_LIFT: CutSpec = {
  ...
  hardware: [
    // v1 เฟี้ยมยก_ตัด D27/D28 = คงที่ 2 ทั้งคู่ (2 บานเสมอ) — เดิมเว็บ HD-640 1 · HD-641 ใช้กฎเฟี้ยมยูโร 7/10
    // 21 ก.ย.69: ใช้รหัสสโตร์ (JR00198=HD-640 · JR00252=HD-641 · JR00203=HD-642 · JR00239=HD-643)
    //   ชื่อยังขึ้น HD-xxx ให้ช่างอ่านเหมือนเดิม · ราคาเท่าเดิมทุกตัว · ฝั่งคิดราคาใช้รหัส JR อยู่แล้ว → ตัดสต็อกตรงรหัส
    { name: "HD-640 บานพับล้อบน", sku: "JR00198", qty: () => 2, unit: "ตัว" },
    { name: "HD-641 บานพับเฟี้ยม", sku: "JR00252", qty: () => 2, unit: "ตัว" },
    { name: "HD-642 บานพับมือจับ", sku: "JR00203", qty: () => 1, unit: "ตัว" },
    { name: "HD-643 บานพับไกด์ล่าง", sku: "JR00239", qty: () => 1, unit: "ตัว" },
    { name: "HD-474 มือจับกลอน", sku: "JR00213", qty: () => 1, unit: "ตัว" },
    { name: "HD-312 ตลับกลอนล็อค", sku: "HD-312", qty: () => 1, unit: "ตัว" },
    ...
    // รหัสยาง v1 + decisions ข้อ 19: ลูกโป่ง JR00273 · อัด JR00783 · รอง JR00258 (เดิม JR00770/768/769)
    { name: "ยางอัด", sku: "JR00783", qty: (o) => Math.round(2 * ((o.W - 25.4) + ((o.H - 8.2) / 2 - 13)) * o.N / 100 * 100) / 100, unit: "ม." },
    { name: "ยางรอง", sku: "JR00258", qty: (o) => Math.round(2 * ((o.W - 25.4) + ((o.H - 8.2) / 2 - 13)) * o.N / 100 * 100) / 100, unit: "ม." },
    // v1 เฟี้ยมยก_ตัด D36 = (2×เฟรมข้าง + เฟรมล่าง + เฟรมบน + 2×(กรอบตั้ง×4 + กรอบนอน×4)) ÷ 100
    //   ตรวจ 200×120 → (2×118 + 189 + 200 + 2×(4×55.9 + 4×187.6))/100 = 25.73 ม. (เดิมเว็บ 9.74)
    { name: "ยางลูกโป่ง 6mm", sku: "JR00273", unit: "ม.",
      qty: (o) => Math.round((2 * (o.H - 2) + (o.W - 11) + o.W + 2 * (4 * ((o.H - 8.2) / 2) + 4 * (o.W - 12.4))) / 100 * 100) / 100 },
  ],
};
...
export const CUT_SPECS: CutSpec[] = [
  ...
  FUJI_SLIDE, FUJI_SLIDE_CENTER, FUJI_SLIDE_MULTI, FUJI_SWING, FUJI_DOOR, FUJI_DOOR_DOUBLE, FUJI_FIX, FUJI_HUNG,
