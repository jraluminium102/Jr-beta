...
| # | ประเด็น | ตัดสินใจ | เหตุผล |
|---|--------|---------|--------|
| **DF1** | ⚠️ premise ของ goal | **(แก้ไขภายหลัง — ดู DF8)** เดิมเชื่อสรุป agent ว่า concept ถูกแล้ว → **ผิด** | ผู้ใช้ตรวจจริงแล้วพบ concept สลับจริง |
| **DF8** | ✅ ผู้ใช้ตรวจจริง: หน้าคิดราคา**ไม่ดึงลูกค้า** (ช่อง `custName` เป็น free-text ใน iframe) · หน้าใบเสนอ/new เป็นที่กดสร้าง+มีดรอปดาวน์ทะเบียน แต่กรอกรายการเอง (ไม่ใช้เครื่องคิดราคา) = **concept สลับจริง** (agent สรุปพลาดเพราะดูแค่ bridge sessionStorage ไม่ได้ดูว่าลูกค้ามาจากไหน) | **แก้ให้ถูก:** (1) หน้าคิดราคา (`CalculatorClient`) เพิ่มดรอปดาวน์ลูกค้า**จากทะเบียน** → แนบ `customer_id` ไปกับราคาที่คิด (2) `QuotationForm` รับ `customer_id` จริง → preselect (3) ปุ่มสร้างในหน้า list ชี้ `/calculator` → create เริ่มที่เครื่องคิดราคา · **ไม่แตะสูตรใน iframe** | บทเรียน: อย่าเชื่อสรุป agent โดยไม่ตรวจ flow ปลายทางจริง (ลูกค้ามาจาก registry หรือ free-text) |
