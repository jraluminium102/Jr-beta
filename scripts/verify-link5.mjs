...
console.log('\n═══ ③ PC Door — รหัส + อุปกรณ์ผูกสโตร์ ═══');
{
  ...
  // 21 ก.ย.69: พอร์ตไฟล์ v1 เปลี่ยน PC Door ไปใช้ "รหัสสโตร์" ทั้งชุด (JR03129/JR03136/JR03141 ฯลฯ)
  //   ด่านเดิมยังไล่หารหัส F79xx ของยุคก่อน — เลิกใช้แล้ว เปลี่ยนมาตรวจว่าทุกเส้นต้องมีรหัสที่ผูกสโตร์ได้
  ok('ทุกเส้นอลูมีรหัส', codes.every((c) => !!c), codes.join(','));
  for (const c of ['JR03129', 'JR03136', 'JR03141', 'B20051', 'B20054'])
    ok(`ใช้รหัส ${c}`, codes.includes(c));
  // มือจับในสูตร = เมโทร (ตามชีตคิดทุน) · ยี่ห้อที่ลูกค้าเลือกจริงมาจากใบตัด เพราะ pcdoor อยู่ใน HW_FROM_CUTLIST
  for (const [name, sku] of [['บานพับ', 'JR00473'], ['ล้อรางบน', 'JR00544'], ['มือจับ กุญแจ', 'JR00368'], ['กลอน', 'JR00630'], ['น็อตเฟรม', 'JR00864']]) {
  ...
  // 21 ก.ย.69: เลข 2,970 เป็นของก่อนพอร์ตไฟล์ v1 · ตอนนี้ทั้งบานเทียบกับชีต "คิดทุน PC Door" ที่ขนาดเดียวกันนี้
  //   แล้วต่างกัน 0.00 (ตัวเทียบชีต w28-parity) → ตรึงเลขใหม่ 4,414 แทน
  ok(`ค่าอุปกรณ์ตรงชีต คิดทุน PC Door (฿${Math.round(r.cost.hardware)})`, Math.abs(r.cost.hardware - 4414) < 1, String(Math.round(r.cost.hardware)));
}
...
console.log('\n═══ ④ Velora + บานติดตาย ═══');
{
  ...
  // 21 ก.ย.69: หลังพอร์ตไฟล์ v1 มีเส้นลูกฟูกเมืองทอง (JR01994) เพิ่มมาด้วย → เช็คว่า "ทุกเส้นเป็นรหัสสโตร์" แทนการล็อก 2 รหัส
  ok('Velora อลูผูกรหัสสโตร์ครบทุกบรรทัด', V.alu.every((a) => /^(JR|B)[0-9]{5}$/.test(String(a.code || ''))), V.alu.map((a) => a.code).join(','));
