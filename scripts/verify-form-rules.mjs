...
console.log('\n═══ ④ ของที่เจ้าของเคาะ 21 ส.ค.69 ═══');
{
  ...
  // 18 ก.ย.69 "ตีให้เท่าไฟล์เด๊ะ ๆ": ชีต คิดทุน SlimLux แถว 18 คิดบังใบ 4 หุน = ราคาสี E191/D191 ตรง ๆ ไม่บวกค่าอบ
  //   → kg = 0 (ไม่เข้ากองค่าอบ) + filePrice ตามสี (ขาว/ดำ/เทา 200 · สีอื่น 170) — เดิมเทสนี้ล็อก 0.9
  ok('กล่อง 4 หุน ไม่เข้ากองค่าอบ (ชีตไม่บวกค่าอบ)', box?.kg === 0, String(box?.kg));
  ok('กล่อง 4 หุน ราคาตามไฟล์ตามสี', /200/.test(String(box?.filePrice)) && /170/.test(String(box?.filePrice)), String(box?.filePrice));
