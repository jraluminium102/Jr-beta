...
console.log("\n═══ ② โครงตรงใบตัด (400×200 · ไวนิล · แปเดี่ยว · ปลาย=รางน้ำ) ═══");
{
  ...
  // v1 + decisions ข้อ 5: ระยะจันทันไวนิล = 100 (เดิม 75) → จันทันรวม = ⌈400/100⌉+1 = 5 · ช่อง 4 · แถวแป = ⌈200/50⌉+1 = 5
  ok("จันทันรวม 5 แนว (ระยะ 100 · v1)", row(r, "จันทันซอย 1.6×4").qty === 5, String(row(r, "จันทันซอย 1.6×4").qty));
...
// ⚠ เวฟ 5 (16 ก.ย.69): ด่านนี้ "แดงโดยรู้ตัว" — คิดทุนรื้อ BOM ตามไฟล์ v1 แล้ว (หลังคา_ตัด #1/#3/#7:
