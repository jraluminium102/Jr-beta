...
console.log("\n═══ ① ค่าที่เจ้าของเคาะ 27 ส.ค.69 ═══");
{
  ...
  // เวฟ 5 (16 ก.ย.69) decisions ข้อ 5: ไวนิล = 100 ตามไฟล์ v1 (คำสั่งเดิม 27 ส.ค.69 ที่ให้ 75 ถูกยกเลิก)
  ok("ระยะจันทัน ไวนิล = 100 (ไฟล์ v1 · เดิมเคาะ 75)", E1("ไวนิล") === 100, String(E1("ไวนิล")));
  ok("วัสดุที่ใบตัดไม่มีชนิดแผ่น (กระจก) ตกไปไวนิล 100 เหมือน from-recipe", E1("กระจก 4+4") === 100, String(E1("กระจก 4+4")));
  ok('กล่องเหล็ก 1"×1" ยังอยู่ (เจ้าของสั่งเก็บไว้)', P.consum.some((c) => /กล่องเหล็ก/.test(c.name)));
  // เวฟ 1 + decisions ข้อ 2: ฝั่ง "คิดทุน" เลิก packBars เปลี่ยนเป็น aluWaste (ตัดเท่าไหร่คิดเท่านั้น × 1.3)
  //   ฝั่งใบตัดยังปัดขึ้นเส้นเต็ม (packBars) เหมือนเดิม — คนละฝั่งกันโดยตั้งใจ
  ok("คิดทุนนับเส้นแบบเศษส่วน ×1.3 (aluWaste) · ใบตัดยังปัดเต็มเส้น (packBars)",
    P.aluWaste === true && !P.packBars && SPEC.packBars === true);
...
console.log("\n═══ ② โครงตรงใบตัด (400×200 · ไวนิล · แปเดี่ยว · ปลาย=รางน้ำ) ═══");
{
  ...
  // v1 + decisions ข้อ 5: ระยะจันทันไวนิล = 100 → จันทันรวม = ⌈400/100⌉+1 = 5 · ช่อง 4 · แถวแป = ⌈200/50⌉+1 = 5
  // เวฟ 7 รื้อใบตัดตามไฟล์: ซอย = จันทันรวม−2 = 3 (เดิมเว็บ 5) · รัดรอบแยกฝั่งบ้าน/ฝั่งปลาย (เดิมรวมเป็น "กว้าง หน้า-หลัง" 2 ท่อน)
  ok("จันทันซอย = จันทันรวม − 2 = 3 ท่อน (หลังคา_ตัด D14)", row(r, "จันทันซอย 1.6×4").qty === 3, String(row(r, "จันทันซอย 1.6×4").qty));
  // ไฟล์โชว์ 94.375 · เอนจินใบตัดปัดความยาวเป็น 1 ตำแหน่ง = 94.4
  ok("จันทันรัดรอบ ฝั่งบ้าน = ยาวเท่าแป 94.4 × 4 ท่อน (D12 = ช่อง)",
    row(r, "จันทันรัดรอบ ฝั่งบ้าน (ยัดในช่อง)").len === 94.4 && row(r, "จันทันรัดรอบ ฝั่งบ้าน (ยัดในช่อง)").qty === 4,
    row(r, "จันทันรัดรอบ ฝั่งบ้าน (ยัดในช่อง)").len + " × " + row(r, "จันทันรัดรอบ ฝั่งบ้าน (ยัดในช่อง)").qty);
  ok("ยื่นเอียงคิดจาก 'ตก สโลป' ไม่ใช่องศา — √(200²+18²) = 200.8 (E9)",
    row(r, "จันทันรัดรอบ (ยื่น ข้าง)").len === 190.6, String(row(r, "จันทันรัดรอบ (ยื่น ข้าง)").len));
  ok("มีแถวใหม่ตามไฟล์: ฝั่งปลาย · ฝาครอบ · กล่องปิดปลาย",
    !!row(r, "จันทันรัดรอบ ฝั่งปลาย") && !!row(r, "ฝาครอบ") && !!row(r, 'กล่องปิดปลาย 1"×4"'));
  ok("แป เดี่ยว = ช่อง 4 × แถว 5 = 20 ท่อน", row(r, "แป (ยัดในช่อง)").qty === 20, String(row(r, "แป (ยัดในช่อง)").qty));
  ok("แป เดี่ยว ใช้กล่อง 1.6×1.6", row(r, "แป (ยัดในช่อง)").code === 'กล่อง 1.6"x1.6"');
  const rd = cut({ W: 400, P: 200, sheet: "ไวนิล", purlin: "แปคู่" });
  ok("แป คู่ = 20 × 2 = 40 ท่อน · กล่อง 1×1½",
    rd.rows.find((x) => x.name === "แป (ยัดในช่อง)").qty === 40 && rd.rows.find((x) => x.name === "แป (ยัดในช่อง)").code === 'กล่อง 1"x1.5"');
}

// ── ③ กวาดทุกคอมบิเนชัน — จำนวนชิ้นต่อรหัส ฝั่งคิดราคาต้องเท่าใบตัด ──
// ✅ เวฟ 7 (16 ก.ย.69): รื้อ AWNING/GABLE_STRAIGHT ในใบตัดตามไฟล์ v1 เรียบร้อย — ด่านนี้กลับมาเขียวแล้ว
// (บันทึกเดิม) เวฟ 5: คิดทุนรื้อ BOM ตามไฟล์ v1 แล้ว (หลังคา_ตัด #1/#3/#7:
//   จันทันรัดรอบแยกฝั่งบ้าน/ฝั่งปลาย · ซอย = จันทันรวม−2) แต่ `cutlist/products.ts` AWNING ยังเป็นโครงเดิม
//   (รัดรอบกว้าง ×2 · ซอย = จันทันรวม) เพราะเวฟ 5 ห้ามแตะ cutlist/* — เวฟ 2 ก็บันทึกไว้ว่า "รื้อ BOM ต้องทำคู่เวฟ 5"
//   → ต้องให้หัวหน้ารื้อ AWNING/GABLE_STRAIGHT ในใบตัดตามไฟล์ v1 แล้วด่านนี้จะกลับมาเขียวเอง (ห้ามลบด่าน)
console.log("\n═══ ③ กวาดทุกรูปแบบ — ชิ้นต่อรหัส ใบตัด = คิดราคา (⚠ รอใบตัดตามเวฟ 5) ═══");
{
...
console.log("\n═══ ④ กฎที่ต้องไม่พัง ═══");
{
  ...
  // ระยะจันทันต่างตามวัสดุ → ชินโคร์ (138) ใช้จันทันน้อยกว่าไวนิล (100 ตาม v1)
  const nr = (m) => cut({ W: 400, P: 200, sheet: m }).rows.find((x) => x.name === "จันทันซอย 1.6×4").qty;
  ok("ชินโคร์ (จันทัน 138) ใช้จันทันน้อยกว่าไวนิล (100)", nr("ชินโคร์ HC") < nr("ไวนิล"), `${nr("ชินโคร์ HC")} < ${nr("ไวนิล")}`);
}
...
console.log("\n═══ ⑤ หลังคาจั่ว — คิดราคา = ใบตัด 'หลังคาจั่วตรง' ═══");
{
  ...
  // เวฟ 5: จั่วต่างจากกันสาด — ชีต "คิดทุน หลังคาจั่ว" B10-B15 นับ ⌈ท่อน ÷ ⌊600/ยาว⌋⌉ ต่อกลุ่ม = packBars
  //   (กันสาดใช้ Σยาว ÷ 600 × 1.3 = aluWaste) · ยึดตามที่แต่ละชีตคิดจริง
  ok("จั่ว: นับเส้นแบบจัดชิ้นลงเส้นจริง (packBars) ทั้งคิดทุนและใบตัด",
    G.packBars === true && GS.packBars === true);
  const gE1 = (material) => new Function("material", `return ${G.vars.E1}`)(material);
  ok("ระยะจันทันไวนิล 100 (ไฟล์ v1 · เดิมเคาะ 75)", gE1("ไวนิล") === 100, String(gE1("ไวนิล")));
  ...
  const gp = (purlin) => computeCutList(GS, { ...GS.defaults, W: 400, D: 200, ridgeH: 150, purlin }, 1).rows.find((x) => x.name === "แป (ยัดในช่อง)").qty;
  ok("แปคู่ ใช้แป 2 เท่าของแปเดี่ยว", gp("แปคู่") === 2 * gp("แปเดี่ยว"), `${gp("แปคู่")} vs ${gp("แปเดี่ยว")}`);
}
...
console.log("\n═══ ⑥ แผ่นไวนิล — 1 แผ่น (ยาว 7 ม.) ตัดได้หลายแถบ ═══");
{
  ...
  ok("ฝาครอบไวนิล = 1 เส้น/แผ่น → 6 (v20.1 E9 หารแผ่นแล้ว · เดิม 16 ต่อแถบ)", cap?.qty === 6, String(cap?.qty));
}

// ── ⑦ ฉาก 6 หุน / แซด 4" / รัดรอบ = อลูมิเนียม มีรหัส (เจ้าของท้วง 27 ส.ค.69 "ไม่ใช่เหล็ก") ──
console.log('\n═══ ⑦ ฉาก 6 หุน · แซด 4" · รัดรอบ = อลูมีรหัส ไม่ใช่เหล็ก ═══');
{
  const alu = PRODUCTS.roof.alu;
  const has = (code) => alu.some((a) => a.code === code);
  ok("ฉาก 6 หุน อยู่ในกลุ่มเส้นอลู + มีรหัส", has("ฉาก 6 หุน"));
  ok('แซด 4" อยู่ในกลุ่มเส้นอลู + มีรหัส', has('แซด 4"'));
  ok("รัดรอบ ผูกรหัสกล่อง 1×1½", alu.some((a) => /^รัดรอบ/.test(a.name) && a.code === 'กล่อง 1"x1.5"'));
  ok("ไม่มีคำว่า (เหล็ก) ห้อยท้าย 3 ตัวนี้แล้ว",
    !PRODUCTS.roof.consum.some((c) => /^(ฉาก 6 หุน|แซด 4)/.test(String(c.name))));
  ok("ผูกคีย์ราคาสโตร์ครบ (ฉาก|6หุน · ตัวZ|4 · กล่อง|1X1.5)",
    ["ฉาก|6หุน", "ตัวZ|4", "กล่อง|1X1.5"].every((b) => alu.some((a) => a.box === b)));

  // ใบตัดต้องออกรหัสเดียวกัน ไม่งั้นหน้าเทียบขึ้น "มีแต่คิดราคา" + หักสต็อกไม่ได้
  const c = cut({ W: 400, P: 200, sheet: "ไวนิล" });
  for (const [nm, code] of [["ฉาก 6 หุน", "ฉาก 6 หุน"], ['แซด 4"', 'แซด 4"'], ["รัดรอบ (หน้า)", 'กล่อง 1"x1.5"']]) {
    ok(`ใบตัด "${nm}" ออกรหัส ${code}`, row(c, nm)?.code === code, String(row(c, nm)?.code));
  }
  // ราคาต้องไม่หล่นเป็น 0 (แซด 4" ยังไม่มีในสโตร์ → ต้องใช้ราคาสำรองในสูตร)
  const r = computeCost(PB, PRODUCTS.roof, { w: 400, h: 200, p: 1, form: "หลังคาเพิง", material: "ไวนิล", color: "white", colorKey: "white", spec: {}, addons: {} });
  ok("ทุกบรรทัดอลูมีราคา > 0 (แซด 4\" ใช้ราคาสำรอง)", r.lines.filter((l) => l.cat === "alu").every((l) => (Number(l.unitPrice) || 0) > 0),
    r.lines.filter((l) => l.cat === "alu" && !(Number(l.unitPrice) > 0)).map((l) => l.name).join(","));
}
