...
console.log("\n═══ ⑦-b suggestInstallments เรนจ์ (21 ก.ค.69) — 300-500k=3งวด ประกัน30k · 500-700k=4งวด ประกัน40k ═══");
{
  ...
  // งวดสุดท้าย = ประกัน 30,000 (จำนวนคงเดิม) แต่ "เลิกใช้คำว่าเงินประกัน" → หัว = ส่วนที่เหลือ (22 ก.ค.69)
  eq("300-500k → 3 งวด 35%/เหลือ/ประกัน 30,000 · เลิกคำว่าเงินประกัน",
    [p4.length, p4.map((x) => x.amount), p4[2].label.includes("เงินประกัน"), p4[2].label.includes("ส่วนที่เหลือ")],
    [3, [140000, 230000, 30000], false, true]);
  ...
  eq("500-700k → 4 งวด 35%/30%/เหลือ/ประกัน 40,000 · เลิกคำว่าเงินประกัน",
    [p6.length, p6.map((x) => x.amount), p6[3].label.includes("เงินประกัน"), p6[3].label.includes("ส่วนที่เหลือ")],
    [4, [210000, 180000, 170000, 40000], false, true]);
}

console.log("\n═══ ⑦-c suggestInstallments คำพูดงวด (เจ้าของเคาะ 22 ก.ค.69) — ไมล์สโตน %ตามจำนวนงวด ═══");
{
  // 4 งวด (500-700k) ไม่มี VAT → หัวงวดล้วน
  const p4 = suggestInstallments(600000);
  eq("4งวด หัว: มัดจำ / 50% / 80% / ส่วนที่เหลือ", p4.map((x) => x.label), [
    "งวดแรก  เบิกมัดจำลูกค้าตกลงว่าจ้างงาน",
    "งวดที่ 2  เบิกวันแรกเข้าติดตั้ง และงานเสร็จแล้ว 50%",
    "งวดที่ 3  เบิกเมื่องานติดตั้งเสร็จแล้ว 80%",
    "งวดสุดท้าย  เบิกส่วนที่เหลือ เมื่อเก็บงานเรียบร้อยแล้ว",
  ]);
  // 5 งวด (>700k) ไม่มี VAT
  const p5 = suggestInstallments(1000000);
  eq("5งวด หัว: มัดจำ / 40% / 60% / 80% / ส่วนที่เหลือ", p5.map((x) => x.label), [
    "งวดแรก  เบิกมัดจำลูกค้าตกลงว่าจ้างงาน",
    "งวดที่ 2  เบิกวันแรกเข้าติดตั้ง และงานเสร็จแล้ว 40%",
    "งวดที่ 3  เบิกเมื่องานติดตั้งเสร็จแล้ว 60%",
    "งวดที่ 4  เบิกเมื่องานติดตั้งเสร็จแล้ว 80%",
    "งวดสุดท้าย  เบิกส่วนที่เหลือ เมื่อเก็บงานเรียบร้อยแล้ว",
  ]);
  // มี VAT → บรรทัดย่อย: งวด 1-3 ค่าวัสดุ · งวดสุดท้าย(ประกัน) ค่าแรง
  const v4 = suggestInstallments(600000, 7);
  eq("4งวด+VAT บรรทัดย่อย: ค่าวัสดุ×3 / ค่าแรง(งวดสุดท้าย)",
    v4.map((x) => x.label.split("\n")[1] ?? ""),
    ["- ค่าวัสดุ (รวมVat)", "- ค่าวัสดุ (รวมVat)", "- ค่าวัสดุ (รวมVat)", "- ค่าแรง (รวมVat)"]);
  // เจ้าของเคาะ 24 ก.ค.69: งวดสุดท้าย = ค่าแรง เสมอ ไม่ว่ากี่งวด (แม้ไม่มีงวดประกัน) · งวดแรก = ค่าวัสดุ
  const v2 = suggestInstallments(80000, 7);
  eq("2งวด+VAT: งวดแรกค่าวัสดุ · งวดสุดท้ายค่าแรง", v2.map((x) => x.label.split("\n")[1] ?? ""), ["- ค่าวัสดุ (รวมVat)", "- ค่าแรง (รวมVat)"]);
}
