...
console.log("\n═══ ⑦-c suggestInstallments คำพูดงวด (เจ้าของเคาะ 22 ก.ค.69) — ไมล์สโตน %ตามจำนวนงวด ═══");
{
  ...
  // เรนจ์เล็กไม่มีงวดประกัน → งวดสุดท้ายเป็น material → บรรทัดย่อยค่าวัสดุ (คงเดิม)
  const v2 = suggestInstallments(80000, 7);
  eq("2งวด+VAT: ค่าวัสดุ ทั้งคู่", v2.map((x) => x.label.split("\n")[1] ?? ""), ["- ค่าวัสดุ (รวมVat)", "- ค่าวัสดุ (รวมVat)"]);
}
