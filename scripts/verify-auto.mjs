...
console.log("\n═══ ④ มอเตอร์ขึ้นตามประเภทบาน (ห้ามขึ้นมั่ว) ═══");
{
  ...
  //   + "บานกระทุ้ง ออพชั่นเซนเซอร์ฝนหาย ถ้าเลือกไม่ตรงเงื่อนไข"
  //   ⇒ เกินพิกัด = คิดราคาตามปกติ ติดคำเตือนที่ชื่อบรรทัด และเซนเซอร์ต้องไม่หาย (เดิมเช็ค === 0)
  ok("banyok: มอเตอร์ 80 กก. เกินพื้นที่ = ยังคิดเงินเซนเซอร์ได้ (เตือนแทนบล็อก)",
    sensorLines("banyok", { motor: "80", rain_sensor: "yes" }) === 1);
...
console.log("\n═══ ⑧ น้ำหนักบาน → เลือกขนาดมอเตอร์เอง ═══");
{
  ...
  // เลือกเองแล้วเกินพิกัด = คิดเงินได้ แต่ต้องติดคำเตือนที่ชื่อบรรทัด
  //   17 ก.ย.69 เจ้าของสั่งให้เลือกได้แม้ไม่ตรงเงื่อนไข (เดิมเช็ค !amount + cat:'warn')
  const over = W("banyok", { w: 300, h: 250, p: 1, addons: { motor: "80" } });
  ok("เลือก 80 กก. เองแต่บานหนักเกิน → คิดเงิน + เตือนเกินพิกัด",
    mline(over).amount > 0 && /เกินพิกัด/.test(mline(over).name || ""), mline(over).name || warn(over));
  ...
  // motorSizeOk ยังเป็นตัวบอก "เกินพิกัดไหม" เหมือนเดิม (ใช้ตัดสินโหมดอัตโนมัติ + ป้ายเตือน)
  //   แต่ 17 ก.ย.69 เป็นต้นไป UI ไม่ใช้มันซ่อนออปชั่นร่วมแล้ว — เลือกรุ่นเองก็ถือว่ามีมอเตอร์
  ok("เลือก 80 เองแต่เกินพิกัด: motorSizeOk ยังบอกว่าเกิน (ไว้ขึ้นป้ายเตือน)", motorSizeOk("80", big.weight, [80, 300]) === false);

  // เผื่อความปลอดภัย 80% ของพิกัด (เจ้าของสั่ง 8 ก.ย.69 "เผื่อ")
  // เวฟ 8 (16 ก.ย.69): บานยกหักค่าเข้ามุมตามไฟล์ v1 → 180×180 น้ำหนักลดจาก 65.94 เหลือ 63.37 กก. (ต่ำกว่าเกณฑ์เผื่อ 64)
  //   เลื่อนจุดทดสอบเป็น 185×185 = 66.53 กก. เพื่อให้ยังทดสอบ "กลไกเผื่อ 80%" ได้จริง
  const mid = W("banyok", { w: 185, h: 185, p: 1, addons: { motor: "auto" } });
  ok("บาน " + mid.weight.total + " กก. เกิน 80% ของ 80 (=64) → ขยับขึ้น 300", /บานยก 300 กก/.test(mline(mid).name || ""), mline(mid).name || warn(mid));
  // 17 ก.ย.69: เกินพิกัดแล้วยังคิดเงิน → คำเตือนย้ายจากบรรทัด cat:'warn' มาอยู่ท้ายชื่อบรรทัดมอเตอร์
  ok("เลือก 80 เองตอนน้ำหนัก 66.53 → เตือนเกินพิกัด (เผื่อแล้วรับได้ 64)",
    /เกินพิกัด/.test(mline(W("banyok", { w: 185, h: 185, p: 1, addons: { motor: "80" } })).name || ""));
  ...
  // 26 ก.ย.69 โมเดลใหม่: ไม่กรอก fixw = ผืนเลื่อนล้วน (มอเตอร์ลากทั้งผืน) → ชุดที่มีส่วนติดตายต้องกรอก fixw
  //   ผืนรวม 550×150 · ฟิก 400 → บานเลื่อน 150×150 = ชุดเดิมที่เคยใช้เป็นตัวอ้างอิงน้ำหนัก
  const rvFix = W("roof_slide", { w: 550, h: 150, p: 1, form: "เลื่อนข้าง", material: "ไวนิล", spec: { fixw: 400 }, addons: { slide_motor: { kw: "auto" } } });
  ok("หลังคาไวนิล: คิดน้ำหนักแผ่นได้ (7 กก./ตร.ม.)", rv.weight.sheet > 0, String(rv.weight.sheet));
  ok("หลังคาเลื่อน: 'น้ำหนักที่ต้องยก' = เฉพาะส่วนเลื่อน ไม่ใช่ทั้งผืน", rvFix.weight.load > 0 && rvFix.weight.load < rvFix.weight.total, rvFix.weight.load + " / " + rvFix.weight.total);
  ok("ผืนเลื่อนล้วน (ไม่มีฟิก): ต้องลากทั้งผืน = น้ำหนักรวม", Math.abs(rv.weight.load - rv.weight.total) < 0.01, rv.weight.load + " / " + rv.weight.total);
  ...
  //   เลื่อน 150×150 ไวนิล: แผ่น 2.25 ตร.ม. × 7 = 15.75 กก. · โครง 12.10 กก. → ลากรวม 27.85 กก.
  // เวฟ 5 (16 ก.ย.69): 32.8 → 27.85 เพราะถอด "กล่องเหล็ก 1×1 (เลื่อน)" ออกตามชีต "คิดทุน หลังคาเลื่อน" K6 = 0
  //   (โน้ตในชีต: "บานเลื่อนไม่ใช้เสาเหล็ก แขวนกับราง") — เหล็ก 4.5 ม. × 6.6/6 = 4.95 กก. ที่หายไปคือก้อนนี้
  // 21 ก.ย.69: 27.85 → 27.71 เพราะน้ำหนักกล่องยึดชีต "น้ำหนักโปรไฟล์" ทั้งชีต (แปคู่ 1×1½ = 1.992 · แซด 4" = 4.062 · แปเดี่ยว 1.6×1.6 = 3.96)
  ok("น้ำหนักโครงคิดตามยาวตัดจริง ไม่ใช่ 6 ม./เส้น", Math.abs(rvFix.weight.load - 27.71) < 0.05, String(rvFix.weight.load));
...
console.log("\n═══ ⑨ มอเตอร์บานกระทุ้ง — รุ่น/ขนาดที่ทำได้ ═══");
{
  ...
  // เลือกรุ่นเองแล้วขนาดไม่เข้าเกณฑ์ = คิดเงินได้ + เตือน + บอกรุ่นที่ใช้ได้
  //   17 ก.ย.69 เจ้าของสั่ง "เลือกได้แม้ไม่ตรงเงื่อนไข" + "เซนเซอร์ฝนต้องไม่หาย"
  //   ⇒ คำเตือนย้ายมาต่อท้ายชื่อบรรทัดชุดออโต้ (เดิมเป็นบรรทัด cat:'warn' แทนราคา)
  const bad = A(100, 100, 1, "choke80");
  ok("เลือกโช้ค 80 เองแต่บานเล็กไป → คิดเงิน + เตือน", mo(bad).amount > 0 && /ทำได้ 90×115/.test(mo(bad).name || ""), (mo(bad).name || wn(bad)).slice(0, 80));
  ok("เตือนแล้วบอกด้วยว่ารุ่นไหนใช้ได้", /ใช้ได้:/.test(mo(bad).name || ""), (mo(bad).name || "").slice(0, 110));
  ok("บานกระทุ้งเลือกรุ่นไม่เข้าเงื่อนไข: เซนเซอร์กันฝนต้องไม่หาย",
    (computeCost(PB, PRODUCTS.awning, { w: 100, h: 100, p: 1, glassType: "เขียว 6มม.", addons: { awn_auto: "choke80", rain_sensor: "yes" } }).lines || [])
      .some((l) => l.cat === "addon" && /เซนเซอร์กันฝน/.test(l.name || "")));
...
console.log("\n═══ ⑳ ลำดับราคาสี — ขาว=ดำ < เทา=ดำซาฮาร่า ≤ แอทแทค < ลายไม้ ═══");
{
  ...
  const order = (id, c) => {
    ...
    // ✅ ลูกฟูก 2 ทาง มีราคาแอทแทคแล้ว (เจ้าของให้เรต 16 ก.ย.69): 203 บ./กก. × 2.544 กก./เส้น = 516.43
