...
console.log('\n═══ ②f F7994 ตบรางล้อ สีเงิน — ราคาเดียวทุกสี ═══');
{
  ...
  check('ค่าอบ SMS สีเทา = เฉพาะ B20001+B20003 (F7994 ไม่ปน)', Math.round(smsBake * 100) / 100, Math.round((barsNeeded(3, 1, 6.4, true) * 1.11 * 6.4 + barsNeeded(2.2, 2, 6.4, true) * 0.942 * 6.4) * 100 * 100) / 100, 0.5);
...
console.log('\n═══ ②g ราคาเส้นแยกสีจริง 6 สี (ไฟล์ v20.1 ชีตราคาสี) ═══');
{
  ...
  //   12 ก.ย.69 เจ้าของเคาะ "คิดค่าอบตามจริง" → ตรึงค่าไว้ ห้ามหลุดกลับไปเป็น 0 อีก
  {
    ...
    check('ระแนงเลื่อน สีอบพิเศษ: ทุนรวม 22,345.65', Math.round(sp.cost.total * 100) / 100, 22345.65, 1);
    check('ระแนงเลื่อน ลายไม้อบพิเศษ: ทุนรวม 22,875.69', Math.round(wsp.cost.total * 100) / 100, 22875.69, 1);
    check('ระแนงเลื่อน สีขาว: ไม่มีค่าอบ (ทุน 14,951.93 เท่าเดิม)', Math.round(wh.cost.total * 100) / 100, 14951.93, 1);
...
console.log("═══ ⑫ ไม่มีรหัสที่สร้างเองหลงเหลือในสูตร (เจ้าของสั่ง 1 ก.ย.69) ═══");
{
  ...
  check("บรรทัดที่รอเจ้าของเติมรหัส (needCode)", need, 62, 0);   // 74 → 71 (3 ก.ย.69): PC Door เสาตั้ง B20051 + เสานอน B20054 ได้รหัสแล้ว · เฟี้ยมยูโรถอดสักหลาด   // 79 → 74: บานกระทุ้งตัด 5 รายการตามใบตัด (เจ้าของเคาะ 1 ก.ย.69)
