...
const ANCHORS = [
  ...
  { id: 'sms_slide', in: { w: 600, h: 300, p: 3, form: 'อิสระ' }, cost: 18708.24 },
  ...
  { id: 'euro_slide', in: { w: 600, h: 300, p: 3, form: 'อิสระ' }, cost: 31083.72 },
  ...
  { id: 'slimlux', in: { w: 200, h: 200, p: 2, form: 'อิสระ' }, cost: 15200.43 },
  ...
  { id: 'open_door', in: { w: 150, h: 200, p: 1, form: 'มีธรณี' }, cost: 7843.16 },   // +38: ธรณีใช้ราคาไฟล์ F7938(B) 1530 แทนราคาสำรอง 1400 (pricebook คีย์ตาม sku สโตร์)   // รื้อตามไฟล์ตัดประกอบ + ใส่ราคาอุปกรณ์ตามชีตถอดทุน v9 (ค่าอุปกรณ์ = 1,638 ตรงไฟล์เป๊ะ)
  ...
  { id: 'awning', in: { w: 40, h: 40, p: 1, form: 'อิสระ' }, cost: 1946.88 },   // สปิงก็อท/ฉาก คงรหัส+ราคาของเว็บไว้ก่อน (รอเจ้าของเคาะ SKU) → ต่างชีต −125.11
  ...
  { id: 'folding', in: { w: 180, h: 280, p: 2, form: '2บาน: รวบเปิดซ้าย (2-0)' }, cost: 18704.49, labor: 'baseXpanel' },   // +1,257 (24 ส.ค.69): เสา/เสากุญแจ/เสากุญแจมือจับ/บังใบ นับจากตาราง config ในใบตัด (SMS240_CFG) แทนที่จะเดา 2×บาน — ของเดิมนับเสาขาด
  ...
  { id: 'topslide', in: { w: 360, h: 240, p: 2, form: 'เลื่อนซ้อน' }, cost: 16841.21 },
  ...
  { id: 'roof', in: { w: 400, h: 200, p: 1, form: 'หลังคาเพิง' }, cost: 22998 },
  ...
  { id: 'pcdoor', in: { w: 150, h: 200, p: 2, form: 'แบ่ง 2' }, cost: 10552.78 },
  // ⚠ เดิม 7,962 = ค่าจากไฟล์ v9 ที่ปัดขึ้นเต็มเส้นทุกบรรทัด · v20 เปลี่ยนเป็นยาวจริง÷6.4×1.3 → 4,297.55
  { id: 'banyok', in: { w: 100, h: 50, p: 1, form: 'เดี่ยว' }, cost: 4309.73, costOnly: true },
  { id: 'fold_euro', in: { w: 180, h: 280, p: 2, form: '2บาน: 2-0 พับข้างเดียว' }, cost: 19073.06, labor: 'baseXpanel' }   /* 3 ก.ย.69 −13.30 = ถอดสักหลาด (เจ้าของสั่ง "ไม่มี") */,   // +815.70 (24 ส.ค.69): แยกเส้นรายท่อนตามใบตัด + อุปกรณ์ HD ผูกราคาสโตร์
  ...
  { id: 'banyok', in: { w: 100, h: 150, p: 1, form: 'เดี่ยว', glassType: 'เขียว 6มม.' }, cost: 5699.05, costOnly: true },
  ...
  { id: 'roof_gable', in: { w: 400, h: 200, p: 1, form: 'หลังคาจั่ว' }, cost: 40241 },
...
const ANCHORS150 = [
  { id: 'sms_slide', in: { p: 2, form: 'อิสระ' }, cost: 5355.28 },   // เวฟ 3: seg ตามระยะตัดจริงในใบตัด (เดิม 5,414.95)
  { id: 'euro_slide', in: { p: 2, form: 'อิสระ' }, cost: 9806.53 },   // เวฟ 3: มือจับเมโทร + ตบกันสาด 2 เส้น + มุมเลื่อน 8/บาน (เดิม 8,491.93)
  ...
  { id: 'open_door', in: { p: 2, form: 'มีธรณี' }, cost: 9922.33 },   // F7938 ราคาไฟล์ 1530 · เวฟ 4: +297 (คิ้วยาวขึ้นตามไฟล์ 80/60 + ตบธรณีเต็มกว้าง + อุปกรณ์ ERP − มือจับใบลอง)
  { id: 'pcdoor', in: { p: 1, form: 'แบ่ง 2', spec: { pcsill: 'มีธรณี', pcsoft: 'ใส่' } }, cost: 8470.44 },   // เวฟ 3: รื้อเป็นเมืองทอง JR031xx ตามไฟล์ v1 (เดิม 6,973.11)
  { id: 'awning', in: { p: 1, form: 'อิสระ' }, cost: 5157.27 },   // เวฟ 4: ระยะตัดตามใบตัด v1 + วิทโก้ 200 + ยาง 2 บรรทัด + แกนล็อค/วาวล์กลับมา (เดิม 5,305.39)
  { id: 'banyok', in: { p: 1, form: 'เดี่ยว' }, cost: 6419.59 },   // ขนาดดีฟอลต์ 100×150 (ยึดใบตัด 4 ก.ย.69)
  { id: 'fixed', in: { p: 1, form: 'กระจกล้วน' }, cost: 4004 },
  { id: 'topslide', in: { p: 2, form: 'เลื่อนซ้อน' }, cost: 12133.46 },   // เวฟ 3: พอร์ต BOM จากใบตัด (เดิม 12,897)
...
console.log('▶ แก้อลู SMS 187→200 (กระจก/อุปกรณ์ต้องนิ่ง):');
{
  ...
  check('ทุนอลู = 12109.84×200/187', r.cost.alu, 12109.84 * 200 / 187, 1);   // เวฟ 3: ฐานขาว SMS ลดเป็น 12,109.84 หลัง seg ตรงระยะตัดจริง (เดิม 12,325.71)
...
const ANCHORS_COLOR = [
  ...
  ['sms_slide', { w: 150, h: 150, p: 2, form: 'อิสระ' }, { white: 5355.28, sahara: 5643.47, woodStock: 7439.34 }],
  ['euro_slide', { w: 150, h: 150, p: 2, form: 'อิสระ' }, { white: 9806.53, sahara: 10778.98, woodStock: 13222.87 }],   // เวฟ 3: มือจับเมโทร + ตบกันสาด 2 เส้น
  // เวฟ 4: ทุกสีขยับตามการแก้ระยะคิ้ว/ตบธรณี/ยาง/อุปกรณ์ ตามไฟล์ v1 — สัดส่วน %เพิ่มจากขาว ยังเท่าเดิม
  ['open_door', { w: 150, h: 150, p: 2, form: 'มีธรณี' }, { white: 9922.33, sahara: 11085.97, woodStock: 13674.18 }],   // sahara ลด: ธรณีใช้ราคาสีสำเร็จจากไฟล์ (1635) แทน ขาว+ค่าอบ
];
...
console.log('\n═══ ②e ราง กันน้ำ / เตี้ย ต้องสลับวัสดุจริง ═══');
{
  ...
  check('รางเตี้ยต้องถูกกว่ารางกันน้ำ (ต่าง 842)', Math.round(out.cost - low.cost), 842, 1);   // เวฟ 3: seg เฟรมล่าง/ตบปิดรางเตี้ย = W−0.044 ตามใบตัด (เดิมต่าง 851)
...
console.log('\n═══ ②f F7994 ตบรางล้อ สีเงิน — ราคาเดียวทุกสี ═══');
{
  ...
  // 12 ก.ย.69: น้ำหนักยึดแคตตาล็อกผู้ผลิต (โฟลเดอร์ อลู) — B20001 หนา 1.5 = 1.110 กก./ม. · B20003 หนา 1.5 = 0.942 กก./ม. (×6.4 = กก./เส้น)
  // เวฟ 3: B20001 (เฟรมบน) seg = W − 0.044 ตามใบตัด SMS_ตัด → ความยาวที่เข้ากองค่าอบลดลงตาม
  check('ค่าอบ SMS สีเทา = เฉพาะ B20001+B20003 (F7994 ไม่ปน)', Math.round(smsBake * 100) / 100, Math.round((barsNeeded(3 - 0.044, 1, 6.4, true) * 1.11 * 6.4 + barsNeeded(2.2, 2, 6.4, true) * 0.942 * 6.4) * 100 * 100) / 100, 0.5);
...
console.log('\n═══ ②g ราคาเส้นแยกสีจริง 6 สี (ไฟล์ v20.1 ชีตราคาสี) ═══');
{
  ...
  check("น้ำหนักต่อเมตรจากแคตตาล็อก (PB.ALUWEIGHT_KGM)", Object.keys(PB.ALUWEIGHT_KGM ?? {}).filter((k) => !k.startsWith("_")).length, 69, 0);
  check("น้ำหนัก กก./เส้น (ตารางรวม)", Object.keys(PB.ALUWEIGHT ?? {}).length, 146, 0);
  check("B20001 หนา 1.5 = 1.110 กก./ม. (แคตตาล็อก SMS)", PB.ALUWEIGHT_KGM?.B20001, 1.11, 0.001);
  check("B20001 กก./เส้น = 1.110 × 6.4", PB.ALUWEIGHT?.B20001, 7.104, 0.002);
  check("F7859 = 0.935 กก./ม. (แคตตาล็อก Fuji)", PB.ALUWEIGHT_KGM?.F7859, 0.935, 0.001);
  ...
  //   12 ก.ย.69 เจ้าของเคาะ "คิดค่าอบตามจริง" → ตรึงค่าไว้ ห้ามหลุดกลับไปเป็น 0 อีก
  {
    ...
    check('ระแนงเลื่อน: น้ำหนักอลูเข้ากองค่าอบแล้ว (เดิม 0)', Math.round(wh.aluKg * 100) / 100, 29.56, 0.05);
    check('ระแนงเลื่อน สีอบพิเศษ: ค่าอบ 5,114.41', Math.round(sp.cost.bake * 100) / 100, 5114.41, 1);
    // เวฟ 3 (16 ก.ย.69): มือจับแตกรายชิ้นตามชีต v1 (Align กุญแจ/ล็อค/ดัมมี่ + แป้นรับล็อค) แทน "Align ×4 + ชุดล็อค" → ทุกสี −151.00
    check('ระแนงเลื่อน สีอบพิเศษ: ทุนรวม 22,194.65', Math.round(sp.cost.total * 100) / 100, 22194.65, 1);
    check('ระแนงเลื่อน ลายไม้อบพิเศษ: ทุนรวม 22,724.69', Math.round(wsp.cost.total * 100) / 100, 22724.69, 1);
    check('ระแนงเลื่อน สีขาว: ไม่มีค่าอบ (ทุน 14,800.93)', Math.round(wh.cost.total * 100) / 100, 14800.93, 1);
    check('ระแนงเลื่อน: น้ำหนักบานคิดได้แล้ว (เลือกมอเตอร์ได้)', Math.round((wh.weight?.total || 0) * 10) / 10, 22.9, 0.2);
  }
  ...
  check("F7986 = 0.214 กก./ม. × 6.4", PB.ALUWEIGHT?.F7986, 1.37, 0.002);
...
console.log("═══ ⑪ บานยก: ยึดใบตัด FUJI HUNG (เจ้าของสั่ง 4 ก.ย.69) ═══");
{
  ...
  okb("B28015 เสาเสริมเฟรมข้าง = 2 ท่อน ยาวเท่าสูง (ตามใบตัด)", g("B28015")?.seg === "H" && g("B28015")?.count === "2",
    JSON.stringify(g("B28015")));
  okb("B28012 คิ้วยึดเสาเกี่ยว = 2 ท่อน (ตามใบตัด)", g("B28012")?.seg === "W" && g("B28012")?.count === "2", JSON.stringify(g("B28012")));
  okb("B28011 กรอบบาน = แนวนอน 4 + แนวตั้ง 4 ท่อน (ตามใบตัด · ยาวรวมเท่าเดิม)",
    gAll("B28011").length === 2 && gAll("B28011").every((a) => a.count === "4")
    && gAll("B28011").some((a) => a.seg === "W") && gAll("B28011").some((a) => a.seg === "H/2"),
