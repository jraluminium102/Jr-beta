...
const ANCHORS = [
  ...
  { id: 'gate', in: { w: 350, h: 180, p: 1, form: 'นอน' }, cost: 50988, costOnly: true },   // +1,540 (24 ส.ค.69): โครงกล่อง 2"×4" ยึดไฟล์ตัดประกอบ = 12.93 ม. → 3 เส้น (สูตรเดิมคิดจากเส้นรอบรูป ได้ 2 เส้น = ตัดไม่พอจริง)
...
console.log('▶ ประตูรั้ว — ระยะ@ ระแนง (350×180 นอน):');
{
  ...
  check('UI default spec = golden 50988', G({ rnFace: '4.06', rnGap: '5', drive: 'มอเตอร์อัตโนมัติ', gaterail: 'รางใหม่' }), 50988, 1);
  check('ระยะ@15 (ห่างขึ้น→ใบน้อยลง→ถูกลง)', G({ rnGap: '15' }), 38788, 1);
  check('ระยะ@2 (ถี่ขึ้น→ใบเยอะ→แพงขึ้น)', G({ rnGap: '2' }), 63188, 1);
}
