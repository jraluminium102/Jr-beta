...
const ANCHORS = [   // 17 ก.ย.69 รอบสอง: B24003 เฟรมล่างเฟี้ยม ชีตคิดทุนใช้ 1,445 (ชีตราคาสีเขียน 1,455) → เฟี้ยม −10
  ...
  { id: 'sms_slide', in: { w: 600, h: 300, p: 3, form: 'อิสระ' }, cost: 18643.89 },
  ...
  { id: 'euro_slide', in: { w: 600, h: 300, p: 3, form: 'อิสระ' }, cost: 32652.76 },
  ...
  { id: 'slimlux', in: { w: 200, h: 200, p: 2, form: 'อิสระ' }, cost: 15166.82 },
  ...
  { id: 'awning', in: { w: 40, h: 40, p: 1, form: 'อิสระ' }, cost: 2043.99 },   // สปิงก็อท/ฉาก คงรหัส+ราคาของเว็บไว้ก่อน (รอเจ้าของเคาะ SKU) → ต่างชีต −125.11
  ...
  { id: 'pcdoor', in: { w: 150, h: 200, p: 2, form: 'แบ่ง 2' }, cost: 10518.83 },
  ...
  { id: 'fold_euro', in: { w: 180, h: 280, p: 2, form: '2บาน: 2-0 พับข้างเดียว' }, cost: 19613.16, labor: 'baseXpanel' }   /* 3 ก.ย.69 −13.30 = ถอดสักหลาด (เจ้าของสั่ง "ไม่มี") */,   // +815.70 (24 ส.ค.69): แยกเส้นรายท่อนตามใบตัด + อุปกรณ์ HD ผูกราคาสโตร์
  ...
  { id: 'roof_gable', in: { w: 400, h: 200, p: 1, form: 'หลังคาจั่ว' }, cost: 34556.5 },
...
const ANCHORS150 = [
  { id: 'sms_slide', in: { p: 2, form: 'อิสระ' }, cost: 5351.97 },   // เวฟ 3: seg ตามระยะตัดจริงในใบตัด (เดิม 5,414.95)
  { id: 'euro_slide', in: { p: 2, form: 'อิสระ' }, cost: 10131.22 },   // เวฟ 7: ความยาวตัดจริงตามแผง ⑦ (เดิม 9,806.53)   // เวฟ 3: มือจับเมโทร + ตบกันสาด 2 เส้น + มุมเลื่อน 8/บาน (เดิม 8,491.93)
  ...
  { id: 'pcdoor', in: { p: 1, form: 'แบ่ง 2', spec: { pcsill: 'มีธรณี', pcsoft: 'ใส่' } }, cost: 8450.3 },   // เวฟ 3: รื้อเป็นเมืองทอง JR031xx ตามไฟล์ v1 (เดิม 6,973.11)
  { id: 'awning', in: { p: 1, form: 'อิสระ' }, cost: 5519.04 },   // เวฟ 4: ระยะตัดตามใบตัด v1 + วิทโก้ 200 + ยาง 2 บรรทัด + แกนล็อค/วาวล์กลับมา (เดิม 5,305.39)
...
const ANCHORS_COLOR = [
  ...
  ['sms_slide', { w: 150, h: 150, p: 2, form: 'อิสระ' }, { white: 5351.97, sahara: 5652.62, woodStock: 7448.48 }],
  ['euro_slide', { w: 150, h: 150, p: 2, form: 'อิสระ' }, { white: 10131.22, sahara: 10555.05, woodStock: 12928.69 }],   // เวฟ 3: มือจับเมโทร + ตบกันสาด 2 เส้น
...
console.log('\n═══ ②g ราคาเส้นแยกสีจริง 6 สี (ไฟล์ v20.1 ชีตราคาสี) ═══');
{
  ...
  //     → กองค่าอบหนักขึ้น ค่าตรึงขยับตาม (สีขาวไม่มีค่าอบ จึงเท่าเดิม 14,800.93)
  {
    ...
    check('ระแนงเลื่อน สีอบพิเศษ: ทุนรวม 23,880.33', Math.round(sp.cost.total * 100) / 100, 23880.33, 1);   // 17 ก.ย.69 ราคายึดไฟล์ (ชีตคิดทุนใช้ราคาชุดนี้จริง) B22001/B22002
    check('ระแนงเลื่อน ลายไม้อบพิเศษ: ทุนรวม 24,586.44', Math.round(wsp.cost.total * 100) / 100, 24586.44, 1);
    check('ระแนงเลื่อน สีขาว: ไม่มีค่าอบ (ทุน 14,694.84)', Math.round(wh.cost.total * 100) / 100, 14694.84, 1);
...
console.log("═══ ⑦ กลาสเฮ้าส์ (เพิงตรง) ═══");
{
  ...
  check("ทุนรวม 400×300 ไวนิล", r.cost.total, 38363, 1);
