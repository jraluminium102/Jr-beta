...
const ANCHORS = [   // 17 ก.ย.69 รอบสอง: B24003 เฟรมล่างเฟี้ยม ชีตคิดทุนใช้ 1,445 (ชีตราคาสีเขียน 1,455) → เฟี้ยม −10
  ...
  // 26 ก.ย.69 เจ้าของนิยาม "กว้าง×ยื่นที่กรอก = ผืนรวม · ใต้บานเลื่อนโล่ง" → ชุดของชีต (ติดตาย 400×200 + บานเลื่อน 150 ×2)
  //   เขียนใหม่เป็น ผืนรวม 700×200 · fixw 400 → ค่าแรง 5,093/10,409 ตรงชีต ⑤⑥ เป๊ะ (โมเดลเดิมได้ 4,922/10,194)
  { id: 'roof_slide', in: { w: 700, h: 200, p: 2, form: 'เลื่อนข้าง', spec: { fixw: 400 }, addons: { slide_motor: { kw: '80' } } }, cost: 69477.4, costOnly: true },   // 63,277 + มอเตอร์ 80 กก. 6,200 (4,500 + ค่าส่ง 1,700 ตรงชีต D13)
...
console.log('\n═══ ②g ราคาเส้นแยกสีจริง 6 สี (ไฟล์ v20.1 ชีตราคาสี) ═══');
{
  ...
  check("น้ำหนักต่อเมตร (PB.ALUWEIGHT_KGM) — ไฟล์ก่อน แคตตาล็อกเป็นตัวสำรอง", Object.keys(PB.ALUWEIGHT_KGM ?? {}).filter((k) => !k.startsWith("_")).length, 102, 0);
  check("น้ำหนัก กก./เส้น (ตารางรวม)", Object.keys(PB.ALUWEIGHT ?? {}).length, 155, 0);
