...
console.log("\n═══ ④b ราคาสำรองจากไฟล์ถอดทุน v9 (เจ้าของสั่ง 19 ส.ค.69: 6 รหัสไม่มีราคาในสโตร์) ═══");
{
  ...
  ok("ราคาไฟล์อุปกรณ์ไม่ไปแตะฝั่งอลู (ไม่มี hardwareLines = เท่าราคาฐาน)",
    computeCost(BASE, PRODUCTS.sms_slide, { ...BASE_IN }).sell.withInstall === 61800, "");
}
...
//   เจ้าของเจอเอง 4 ก.ย.69: SMS คิดราคาผูก JR00228 แต่ใบตัดผูก JR00576
{
  ...
  const RETIRED = {
    ...
    JR00242: "สักหลาด ตัวเก่า — ย้ายไป JR00794 สักหลาด 5×3 (4 ก.ย.69)",
    JR00226: "น็อต ตัวเก่า — ย้ายไป JR00864 น็อต 1 นิ้ว (4 ก.ย.69)",
    JR00244: "ยาง ตัวเก่า — ย้ายไป JR00771 ยาง epdm sponge (4 ก.ย.69)",
    JR00267: "ฉากประคองมุม — รวมเป็น JR00480 ฉากประกอบมุม (4 ก.ย.69)",
    JR00200: "ฉากประกอบมุม ตัวเก่า — รวมเป็น JR00480 (4 ก.ย.69)",
    JR00276: "เจ้าของสั่งลบ — ใช้ JR00592 สปิงก็อท แทน (4 ก.ย.69)",
    JR00243: "เจ้าของสั่งลบ — ใช้ JR00564 แทน (4 ก.ย.69)",
  };
...
//   เจ้าของท้วง 4 ก.ย.69: อุปกรณ์บานเฟี้ยม SMS ติดแท็กแค่ "ใช้ในใบตัด" ทั้งที่คิดราคาก็ใช้
//   สาเหตุ: 10 รุ่นใน HW_FROM_CUTLIST ดึงรายการอุปกรณ์จากใบตัดมาคิดเงิน แต่แท็กดูแค่สูตรใน products.mjs
{
  const { calcLink } = await import("../src/lib/calculator40/stock-link.ts");
  const tagged = (sku, name = "") => calcLink({ sku, name }).linked;
  // อุปกรณ์บานเฟี้ยม SMS ทั้ง 16 รหัส (ดำ JR00602-609 · เงิน JR00610-617)
  const foldSkus = ["JR00602","JR00603","JR00604","JR00605","JR00606","JR00607","JR00608","JR00609",
                    "JR00610","JR00611","JR00612","JR00613","JR00614","JR00615","JR00616","JR00617"];
  ok("อุปกรณ์เฟี้ยม SMS ทั้ง 16 รหัส ติดแท็กใช้ในคิดราคา", foldSkus.every((s) => tagged(s)),
    foldSkus.filter((s) => !tagged(s)).join(",") || "-");
  // อุปกรณ์ที่สูตรผูกด้วยรหัสตรง ๆ
  for (const s of ["JR00576", "JR00794", "JR00864", "JR00771", "JR00480", "JR00596", "JR00598"])
...
//   เจ้าของท้วง 4 ก.ย.69 "ทำไมทำงานแบบนี้ ที่ผ่านมาก็มั่วสิ"
//   30 บรรทัดในสูตรยังผูกราคาด้วยชื่อ (PARTS/GLASS/ROOFMAT/MOTOR/EXTRA)
//   กติกา: แถวที่สร้างไว้เพื่อการนี้ (supplier = ถอดทุน R4.0) ชนะเสมอ ไม่ว่าแถวใหม่จะมาก่อนหรือหลัง
{
  const { buildPriceOverride, calcLink } = await import("../src/lib/calculator40/stock-link.ts");
  const owner = { name: "มือจับ", sku: "JR00251", unit_cost: 1500, supplier: "ถอดทุน R4.0" };
  const impostor = { name: "มือจับ", sku: "JR09999", unit_cost: 35, supplier: "ร้านอื่น" };
  ok("ของใหม่ชื่อชน มาทีหลัง ต้องแย่งราคาไม่ได้", buildPriceOverride([owner, impostor], BASE).PARTS["มือจับ"] === 1500);
  ok("ของใหม่ชื่อชน มาก่อน ก็ต้องแย่งไม่ได้", buildPriceOverride([impostor, owner], BASE).PARTS["มือจับ"] === 1500);
  ok("ไม่มีแถวของจริง = ยังผูกให้ (ราคาไม่หายเงียบ)", buildPriceOverride([impostor], BASE).PARTS["มือจับ"] === 35);
  // แท็กต้องบอกความจริง: ชื่อที่ไม่อยู่ในตารางราคา = สโตร์ไม่เกี่ยว ห้ามติดแท็ก
  ok("ชื่อกว้างที่ไม่อยู่ในตารางราคา (เสา) ต้องไม่ติดแท็กคิดราคา", !calcLink({ sku: "JR09998", name: "เสา" }).linked);
  ok("ชื่อที่อยู่ในตารางราคาจริง (มือจับ) ต้องติดแท็ก", calcLink({ sku: "JR00251", name: "มือจับ" }).linked);
}
