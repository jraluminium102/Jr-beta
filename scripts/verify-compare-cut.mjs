...
console.log("\n═══ ③b โหนกเกี่ยว ออโต้ตามความสูง (คิดราคา ↔ ใบตัด ต้องตรงกัน) ═══");
{
  ...
  for (const h of [150, 200, 239]) {
    const r = at(h);
    ok(`สูง ${h} ซม. (ต่ำกว่า 240) → ใช้เสาเกี่ยวธรรมดา ตรงกันสองฝั่ง`, row(r, 'B20009')?.status === 'ตรง', row(r, 'B20009')?.status);
    ok(`สูง ${h} ซม. → ไม่มีเสาเกี่ยวรับแรงทั้งสองฝั่ง`, !row(r, 'B20010'), JSON.stringify(row(r, 'B20010') ?? {}));
  }
  for (const h of [240, 280, 300]) {
    const r = at(h);
    ok(`สูง ${h} ซม. (ตั้งแต่ 240) → เสาเกี่ยวรับแรง B20010 ตรงกันสองฝั่ง`, row(r, 'B20010')?.status === 'ตรง', row(r, 'B20010')?.status);
    ok(`สูง ${h} ซม. → เสาเกี่ยวธรรมดาก็ต้องตรงด้วย`, row(r, 'B20009')?.status === 'ตรง', row(r, 'B20009')?.status);
  }
  // ใบตัด v1 ใช้ IF(สูง >= 240) → 240 นับเป็นโหนก (เวฟ 2 แก้ honkOf จาก > เป็น >=)
  ok('เส้นแบ่งอยู่ที่ 240 พอดี (239 = ธรรมดา · 240 = โหนก ตามใบตัด v1)',
    !row(at(239), 'B20010') && row(at(240), 'B20010')?.cutBars > 0, '');
...
console.log("\n═══ ③c SMS บานเลื่อน — คิดราคา = ใบตัด ทุกเคส ═══");
{
  ...
  // ไฟล์ v1 (SMS_ตัด E20): ตบรางล้อ = จำนวน "บานเลื่อน" → ลากจูง 3 บาน = 2 ชิ้น (เดิมเว็บ 3)
  ok("ลากจูง 3 บาน: ตบรางล้อ 2 ชิ้น (= บานเลื่อน N−1 ตามไฟล์ v1)",
    bar(at("ลากจูง", 3, "รางกันน้ำ"), "F7994")?.calcPieces === 2, "");
...
console.log("\n═══ ④ก หน้าเทียบ: อุปกรณ์ต้องมาจากใบตัด + ห้ามขึ้นเขียวตอนยังไม่มีราคา ═══");
{
  // เวฟ 1 (16 ก.ย.69) เติม PB.HWPRICE 175 รหัสจากชีต "ราคา ERP" → เฟี้ยมมีราคาครบแล้ว
  //   engine จึงใช้รายการอุปกรณ์จากใบตัดจริง (เดิมถอยไปใช้ก้อนเหมา "อุปกรณ์เฟี้ยม (ชุด)")
  const P = PRODUCTS.folding, d = P.defaults;
  const inFold = { prodId: "folding", w: d.w, h: d.h, p: d.p || 2, form: P.defForm, spec: {}, cut: {} };
  const r = compareCut(PB, inFold);
  ok("บานเฟี้ยม: ใช้รายการอุปกรณ์จากใบตัด (ไม่ถอยไปสูตรเก่า)", r.hwFromCutlist === true, String(r.hwFromCutlist));
  ok("ไม่มีรหัสค้าง 'รอเติมราคา'", r.hwPendingPrice !== true && (r.hwMissing?.length ?? 0) === 0, String(r.hwMissing?.length));
  ...
  // ⚠ หัวใจกันบั๊กเดิม ยังต้องอยู่: จำลองว่ารหัสหนึ่งยังไม่มีราคา → ห้ามขึ้น "ตรง" เด็ดขาด
  const PB_NOPRICE = JSON.parse(JSON.stringify(PB));
  delete PB_NOPRICE.HWPRICE.JR00563;
  if (PB_NOPRICE.SKUPRICE) delete PB_NOPRICE.SKUPRICE.JR00563;
  const r2 = compareCut(PB_NOPRICE, inFold);
  const pendSku = new Set((r2.hwMissing ?? []).map((m) => String(m.sku || "").toUpperCase()));
  const lying = r2.hardware.filter((h) => pendSku.has(String(h.sku || "").toUpperCase()) && h.status === "ตรง");
  ok("ถอดราคา JR00563 → ต้องรู้ตัวว่ายังไม่มีราคา", (r2.hwMissing?.length ?? 0) > 0, String(r2.hwMissing?.length));
...
console.log("\n═══ ④ข รางน้ำอลู ผูกสโตร์ 'กล่องเปิด 4\"' — แก้ราคาที่สโตร์แล้วทุนต้องขยับ ═══");
{
  ...
  // 18 ก.ย.69 ราคาไฟล์มาก่อนสโตร์ (เจ้าของสั่ง "ตีให้เท่าไฟล์เด๊ะ ๆ") → ด่านนี้ตรวจ "การผูกสโตร์" จึงวัดในโหมดสโตร์
  //   (ถ้าเจ้าของสลับกลับ PB.PRICE_SOURCE = "store" การผูกนี้ต้องยังทำงาน)
  PB_BOX.PRICE_SOURCE = "store";
  const PB_STORE = { ...PB, PRICE_SOURCE: "store" };
  for (const id of ["glasshouse", "glasshouse_multi", "roof_multi", "gable_multi", "roof"]) {
    ...
    const base = compareCut(PB_STORE, IN2)?.totals?.costTotal;
...
console.log("\n═══ เสารับแรง F7951 สูงเกิน 2.6 ม. (คิดราคา + ใบตัด) ═══");
{
  ...
  ok("คิดราคา สูง 2.8 ม. → ยังไม่มี (ไฟล์ v1 เกณฑ์คือ 'เกิน' 2.8)", postOf(280).post === 0, String(postOf(280).post));
  ok("คิดราคา สูง 2.9 ม. → มีเสารับแรง 1 ต้น (ไฟล์ v1 · เดิมเว็บ 4 ต้นที่ 2.6)", postOf(290).post === 1, String(postOf(290).post));
  const at300 = postOf(300);
  // เจ้าของเคาะ 16 ก.ย.69 (ข้อ 6 "ยูโร ตามไฟล์ทั้งหมด"): เสารับแรง = 1 ต้น ยาวเท่าสูงช่อง เมื่อสูงเกิน 280
  // (ของเดิม 4 ต้นที่ 2.6 ม. เว็บคิดเกินไฟล์ ~3,600 บาท/ชุด) · จำนวนตบเกี่ยวยึดไฟล์ → เพิ่มเท่าตบเกี่ยว
  //   ⚠ รอเจ้าของยืนยัน: "เพิ่มเข้ามา" (4+4) หรือ "แบ่งครึ่ง" (2+2)
  ok("คิดราคา 600×300 3 บาน: ตบเกี่ยวตามไฟล์ 4 + เสารับแรง 1 (ยูโร_ตัด แถว 43)",
    at300.hook === 4 && at300.post === 1, "ตบเกี่ยว " + at300.hook + " · เสารับแรง " + at300.post);
  ...
  ok("ใบตัด สูง 3.0 ม. → เสารับแรง 1 ต้น (ไฟล์ v1)", cutAt(300).post?.qty === 1, String(cutAt(300).post?.qty));
  ok("ใบตัด เสารับแรง ตัดยาว = สูงช่องเต็ม (ไม่ใช่ยาวเท่าตบเกี่ยว)", cutAt(300).post?.len === 300, String(cutAt(300).post?.len));
}
