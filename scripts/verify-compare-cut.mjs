...
console.log("\n═══ ③ จับ 'ไม่ตรงกัน' ได้จริง (ไม่ใช่ขึ้นเขียวหมดทุกกรณี) ═══");
{
  ...
  ok("รายชื่อรุ่นที่เทียบได้ ตรงกับที่ from-recipe รองรับ", COMPARABLE.length === 11, String(COMPARABLE.length));   // +fold_euro +fold_lift (21 ส.ค.69) · +roof กันสาด ดึงขึ้นดิบ (26 ส.ค.69)
