...
console.log("\n═══ ยูโร (FUJI) — ค่าของอุปกรณ์ต้องมาจากใบตัด ═══");
{
  ...
  //   เจ้าของถาม 20 ส.ค.69: ป้าย "นับคนละหน่วย" เดิมขึ้นทุกบรรทัดพวกนี้ ทั้งที่เส้นตรงกันอยู่แล้ว
  {
    ...
    ok("บรรทัดนับเป็นเส้น: ช่องชิ้นว่าง (ไม่เอาไปเทียบ)", [ "F7983", "F7951", "F7986", "F7992" ].every((c) => !row(c)?.calcPieces), "");
    ok("บรรทัดนับเป็นเส้น: เส้นตรงกัน → ขึ้น 'ตรง' ไม่ใช่แดงหลอก",
    ...
    // F7988 คิด 7 เส้น แต่ตัด 2 ท่อน/เส้น → ใช้จริง 4 เส้น = ของจริงที่ต้องแดง
    ok("F7988 ยังฟ้องว่าคิดเกิน (7 เส้น แต่ใช้จริง 4)", row("F7988")?.status === "จำนวนต่าง", row("F7988")?.status);
  }
