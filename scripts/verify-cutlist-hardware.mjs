...
// ── PC Door · default W300 H240 แบ่ง2 (pcN=2 · บานเลื่อน=1) · ซ้าย=กุญแจ+ล็อค ขวา=ล็อค+ดัมมี่ ──
{
  ...
  check("PC", res, [
    ...
    // v1 (16 ก.ย.69): PCDoor_ตัด #13 หัวต่อราง = 1 แถว รหัสตามด้าน (ซ้าย JR02969 ค่าตั้งต้น)
    //   แถว "ฝาครอบราง" (อุปกรณ์) ถอดออก — v1 นับเป็นโปรไฟล์ "ฝาครอบรางบน" JR03136 ×1 ท่อนแล้ว (เดิมนับซ้ำ 2 ที่)
    { nameHas: "หัวต่อราง", sku: "JR02969", qty: 1 },
...
// ── บานโซลิด · default W120 H279 N2 แม่-ลูก motherW80 มีธรณี · ขาว ล็อคปกติ เปิดออก · แม่=คิงโบล็อค+กุญแจ ลูก=ไม่ใส่ ──
{
  ...
  check("โซลิด", res, [
    ...
    // 21 ก.ย.69: ไฟล์เขียน JR00482/JR00557 แต่สโตร์ไม่มี 2 รหัสนี้ (ตรวจย้อนหลัง 1 ก.ย. ⑧)

    //   → รหัสยึดสโตร์ JR00592/JR00480 · ราคายังยึดไฟล์ 12/5 (HWPRICE_BY_PROD.bansolid)
