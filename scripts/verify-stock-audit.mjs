...
console.log("\n═══ ② จับคู่ด้วยรหัส (sku) — อลูรายเส้น ═══");
{
  ...
  // ⚠ sms_slide เขียนรหัสผิด (B20001) ระบบชี้ไป B22001 — ต้องบอกให้เห็นในหมายเหตุ
  const alias = rows.filter((x) => /สูตรเขียนรหัส/.test(x.note));
  ok("เตือนเมื่อสูตรเขียนรหัสไม่ตรงกับที่ระบบใช้จริง", alias.length > 0, "");
}
