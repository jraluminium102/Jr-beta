...
console.log("\n═══ ① ทุนแต่ละก้อน = ชีตถอดทุน (ไวนิล · 400×200 + เลื่อน 150×150 ×2) ═══");
{
  ...
  ok("ติดตาย (วัสดุ) 24,090 (ชีต 24,390 − กล่องเหล็ก 300)", sum(r, /ติดตาย/) === 24090, String(sum(r, /ติดตาย/)));
  ok("เลื่อน (วัสดุ) 23,980 (ชีต 24,340 − กล่องเหล็ก 360)", sum(r, /เลื่อน\)/) === 23980, String(sum(r, /เลื่อน\)/)));
...
console.log("\n═══ ③ แผ่นไวนิล — 1 แผ่น (ยาว 7 ม.) ตัดได้หลายแถบ ═══");
{
  ...
  ok("ฝาครอบไวนิล ยังนับต่อแถบ 16 (ชีต H8 ไม่หารแผ่น)", line(C(), /^ฝาครอบไวนิล \(ติดตาย\)/)?.qty === 16, String(line(C(), /^ฝาครอบไวนิล \(ติดตาย\)/)?.qty));
}
