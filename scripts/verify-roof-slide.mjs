...
console.log("\n═══ ① ทุนแต่ละก้อน = ชีตถอดทุน (ไวนิล · 400×200 + เลื่อน 150×150 ×2) ═══");
{
  ...
  ok("เลื่อน (วัสดุ) 15,022 (2 บาน · กว้างต่อบานตามที่เว็บถาม)", sum(r, /เลื่อน\)/) === 15022, String(sum(r, /เลื่อน\)/)));
  ...
  ok("ค่าแรงผลิต 4,922 = 3,500 + 113.75×12.5", Math.round(r.labor.prod) === 4922, String(Math.round(r.labor.prod)));
  ok("ค่าแรงติดตั้ง 10,194 = 8,400 + 143.5×12.5", Math.round(r.labor.install) === 10194, String(Math.round(r.labor.install)));
}
...
console.log("\n═══ ④ ค่าแรงคิดจากพื้นที่รวม (ชีต D14/D15 = เรต × (H3 + J3×บาน)) ═══");
{
  ...
  ok("ค่าแรงผลิต = ฐาน + เรต × 12.5 ตร.ม. (ตามตารางกลาง)", Math.round(r.labor.prod) === Math.round(rate.pBase + rate.pRate * 12.5));
