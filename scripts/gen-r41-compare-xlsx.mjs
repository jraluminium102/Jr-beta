...
const HEAD = ["รุ่น (ตาราง R4.1)", "รหัสรุ่น", "ขนาด", "บาน",
  ...
  "ทุนรวม — ตาราง", "ทุนรวม — เว็บ", "สถานะ", "หมายเหตุ"];
const out = [], bad = [];
for (const r of rows) {
  ...
  const row = [r.head, r.id, size, r.p || "", n0(sT), n0(webT), n0(d), dp, n0(r.pdf?.sP), n0(c.sell?.parts?.prod),
    n0(r.pdf?.sI), n0(c.sell?.parts?.inst), n0(r.pdf?.cT), n0(c.cost?.total), st, r.note || ""];
  out.push(row);
...
const sty = (rs) => [0, ...rs.map((r) => STY[r[14]] ?? 0)];
const W = [30, 14, 12, 6, 14, 13, 11, 8, 16, 15, 17, 16, 18, 12, 16, 40];
// สรุปต่อรุ่น
...
for (const r of out) {
  ...
  e.n++;
  if (r[14] === "ตรง") e.ok++; else if (r[14] === "ต่างน้อยกว่า 5%") e.near++;
  else if (r[14] === "ต่างเกิน 5%") e.off++; else e.na++;
  byProd.set(k, e);
}
...
const c = (k) => out.filter((r) => r[14] === k).length;
