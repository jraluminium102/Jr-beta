...
// ===== R3-1: casement_euro — Linear ต่อบาน + convention B 2026-06-13 (กรอกกว้างรวม ÷ บาน) =====
// กว้างรวม 1.6 ÷ 2 บาน = 0.8/บาน → พื้นที่/บาน 0.8×2.2=1.76 → 15,000+max(0,1.76-1.2)×4,166.67 = 17,333.33 → ปัดร้อย 17,400/บาน × 2 = 34,800
const d1 = addRow("1", "casement_euro", "1.6", "2.2", 2);
const sell2 = w.readItem(d1).r.sell;
// เพิ่มเป็น 3 บาน + ขยายกว้างรวมตาม (2.4 ÷ 3 = 0.8/บาน เท่าเดิม) → 17,400 × 3 = 52,200 (เพิ่ม 1 บาน = +17,400)
sf(d1, ".i-w", "2.4"); sf(d1, ".i-panels", "3");
const sell3 = w.readItem(d1).r.sell;
want("R3-1→Linear convB: 2 บาน (กว้างรวม 1.6 ÷2 = 0.8/บาน) = 34,800", sell2 === 34800, "sell2=" + sell2);
want("R3-1→Linear convB: 3 บาน (กว้างรวม 2.4 ÷3 = 0.8/บาน) = 52,200 · เพิ่ม 1 บาน +17,400", sell3 === 52200 && (sell3 - sell2) === 17400, "sell3=" + sell3 + " Δ=" + (sell3 - sell2));
